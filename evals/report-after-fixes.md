...
**3.** `fire` · *Lighting and feeding* (hybrid=0.80, kw=0.85, vec=0.78) — `fire.building-a-fire#3`
...
**1.** `fire` · *The international distress pattern* (hybrid=0.81, kw=1.00, vec=0.73) — `fire.signal-fire#0`
...
**2.** `trip-basics` · *Minimize campfire impacts* (hybrid=0.76, kw=0.78, vec=0.75) — `trip-basics.leave-no-trace#3`
...
**3.** `fire` · *Overview* (hybrid=0.73, kw=0.56, vec=0.80) — `fire.leave-no-trace-fire#0`
...
**2.** `fire` · *When you'd actually need this* (hybrid=0.77, kw=0.76, vec=0.78) — `fire.primitive-fire-starting#0`
  > Primitive fire-starting — making fire with sticks and friction — is a survival skill, not a routine method. It is hard, time-consuming, and fails far more often than it succeeds even with practice. If you have any modern alternative (a f…
...
**2.** `fire` · *Lighting in worst conditions* (hybrid=0.80, kw=0.78, vec=0.80) — `fire.ignition-sources#5`
...
**3.** `fire` · *Be patient and feed it* (hybrid=0.75, kw=0.68, vec=0.79) — `fire.wet-conditions#3`
...
**3.** `fire` · *When you'd actually need this* (hybrid=0.77, kw=0.80, vec=0.76) — `fire.primitive-fire-starting#0`
  > Primitive fire-starting — making fire with sticks and friction — is a survival skill, not a routine method. It is hard, time-consuming, and fails far more often than it succeeds even with practice. If you have any modern alternative (a f…
...
**3.** `fire` · *Overview* (hybrid=0.66, kw=0.41, vec=0.77) — `fire.leave-no-trace-fire#0`
  > Campfires leave the most lasting damage of any common backcountry activity: blackened rocks, sterilized soil, deadwood-stripped forests, and — in worst cases — wildfire. The Leave No Trace ethic does not say "no fires." It says: have one…
...
**2.** `fire` · *Use a stove* (hybrid=0.74, kw=0.75, vec=0.73) — `fire.leave-no-trace-fire#1`
...
**3.** `fire` · *Pre-built ready-to-light fires* (hybrid=0.73, kw=0.69, vec=0.74) — `fire.signal-fire#3`
...
**1.** `gear-fixes` · *Stove won't light* (hybrid=0.81, kw=0.99, vec=0.73) — `gear-fixes.field-repairs#3`
...
**3.** `fire` · *Adapting to wet conditions* (hybrid=0.70, kw=0.60, vec=0.74) — `fire.tinder-and-fire-starting#5`
...
**1.** `trip-basics` · *Cooking and camp chores* (hybrid=0.78, kw=0.85, vec=0.75) — `trip-basics.group-dynamics#3`
  > Distribute camp work. Common split: one person filters water, another lights stove, third sets up tents, fourth deals with food prep. Cleaning shouldn't fall on the cook. Make this explicit at camp — silent expectations breed resentment.…

**2.** `trip-basics` · *Travel and camp on durable surfaces* (hybrid=0.78, kw=0.82, vec=0.75) — `trip-basics.leave-no-trace#1`
...
**3.** `shelter` · *Wildlife corridors* (hybrid=0.76, kw=0.81, vec=0.74) — `shelter.site-selection#5`
...
**3.** `shelter` · *How to pitch a tent in heavy wind* (hybrid=0.75, kw=0.75, vec=0.75) — `shelter.tent-setup#3`
  > If wind is up, orient the lowest, smallest end of the tent into the wind — usually the foot end. Use every guyline the tent has, double up stakes for the most stressed corners, and tension all guys equally so the fabric doesn't develop w…
...
**3.** `weather` · *Why a 10-foot move changes the temperature* (hybrid=0.75, kw=0.91, vec=0.68) — `weather.heat-and-cold-microclimates#0`
...
**2.** `shelter` · *Insulation under you* (hybrid=0.76, kw=0.72, vec=0.78) — `shelter.hammock-camping#2`
...
**1.** `water` · *Sources to avoid even after treatment* (hybrid=0.77, kw=0.83, vec=0.74) — `water.unsafe-water-sources#2`
...
**2.** `water` · *Practical priorities* (hybrid=0.71, kw=0.55, vec=0.77) — `water.unsafe-water-sources#4`
...
**3.** `food` · *Eat early, eat often* (hybrid=0.70, kw=0.95, vec=0.59) — `food.calorie-and-nutrition#3`
...
**1.** `water` · *Sources to avoid even after treatment* (hybrid=0.80, kw=0.90, vec=0.76) — `water.unsafe-water-sources#2`
  > Some contamination cannot be removed by filtration or boiling. Avoid these unless absolutely no alternative exists, and even then consider the trade-offs: - **Stagnant water with algae blooms** (green/red scum, foul smell). Cyanobacteria…
...
**3.** `water` · *Boiling* (hybrid=0.76, kw=0.62, vec=0.82) — `water.purification-methods#0`
...
**3.** `first-aid` · *When to evacuate* (hybrid=0.61, kw=0.54, vec=0.64) — `first-aid.gi-illness#2`
...
**2.** `water` · *Sources that look safer than they are* (hybrid=0.62, kw=0.56, vec=0.65) — `water.unsafe-water-sources#1`
...
**3.** `water` · *Practical priorities* (hybrid=0.60, kw=0.37, vec=0.71) — `water.unsafe-water-sources#4`
  > 1. Best: moving water from springs or mid-stream, in remote drainages, away from grazing, mining, and downstream of campsites. 2. Acceptable: most alpine streams and lakes after proper treatment. 3. Last resort: cloudy or silty water — p…
...
**1.** `first-aid` · *Recognize life-threatening bleeding* (hybrid=0.80, kw=0.83, vec=0.79) — `first-aid.bleeding-control#0`
...
**2.** `first-aid` · *When to evacuate* (hybrid=0.75, kw=0.90, vec=0.68) — `first-aid.nosebleed#3`
...
**3.** `first-aid` · *What to avoid* (hybrid=0.72, kw=0.61, vec=0.77) — `first-aid.bleeding-control#3`
...
**1.** `first-aid` · *Recognize the signs* (hybrid=0.85, kw=0.91, vec=0.82) — `first-aid.hypothermia#0`
...
**2.** `first-aid` · *Rewarm gently* (hybrid=0.71, kw=0.60, vec=0.75) — `first-aid.hypothermia#2`
...
**2.** `first-aid` · *Anaphylaxis (severe allergic reaction)* (hybrid=0.71, kw=0.80, vec=0.68) — `first-aid.bites-stings#3`
...
**3.** `first-aid` · *Adult / older child: abdominal thrusts (Heimlich)* (hybrid=0.66, kw=0.36, vec=0.79) — `first-aid.choking#1`
...
**2.** `first-aid` · *This is not training* (hybrid=0.75, kw=0.66, vec=0.79) — `first-aid.cpr-basics#0`
...
**3.** `first-aid` · *Once you have the person out* (hybrid=0.67, kw=0.50, vec=0.74) — `first-aid.drowning#2`
...
**3.** `navigation` · *Aim-off* (hybrid=0.59, kw=0.67, vec=0.56) — `navigation.handrails-and-aim-off#2`
...
**2.** `first-aid` · *Telling sprains from fractures in the field* (hybrid=0.66, kw=0.68, vec=0.65) — `first-aid.sprains-fractures#0`
...
**3.** `first-aid` · *Splinting a suspected fracture* (hybrid=0.65, kw=0.58, vec=0.68) — `first-aid.sprains-fractures#2`
...
**2.** `water` · *E. coli and other bacteria* (hybrid=0.74, kw=0.76, vec=0.73) — `water.water-borne-illness#2`
...
**3.** `water` · *Giardia* (hybrid=0.69, kw=0.73, vec=0.68) — `water.water-borne-illness#0`
  > Giardia is a single-celled parasite present in most North American surface water, including high alpine streams. Symptoms typically appear 1–3 weeks after exposure: foul-smelling diarrhea, gas, abdominal cramps, nausea, fatigue, weight l…
...
**2.** `wildlife` · *If you see a bear at a distance* (hybrid=0.87, kw=0.94, vec=0.84) — `wildlife.bear-encounter#0`
...
**3.** `wildlife` · *If a moose charges* (hybrid=0.77, kw=0.83, vec=0.74) — `wildlife.moose-and-large-ungulates#2`
...
**1.** `wildlife` · *If you see a bear at a distance* (hybrid=0.75, kw=0.55, vec=0.83) — `wildlife.bears#2`
  > Stop and assess. If the bear hasn't seen you, back away quietly the way you came and detour widely. If the bear has seen you, identify yourself as human: speak calmly, wave your arms slowly. Do not run; running triggers chase instinct in…

**2.** `wildlife` · *When to deploy* (hybrid=0.75, kw=0.61, vec=0.80) — `wildlife.bear-spray#3`
  > Deploy when a bear is charging, approaching aggressively, or has made contact. Do not spray a bear that is calmly grazing or moving away — back away yourself.
...
**2.** `wildlife` · *Coyotes* (hybrid=0.77, kw=0.73, vec=0.78) — `wildlife.coyotes-and-wolves#0`
...
**2.** `wildlife` · *In the water* (hybrid=0.63, kw=0.39, vec=0.74) — `wildlife.alligators-and-crocodiles#4`
...
**3.** `wildlife` · *If attacked* (hybrid=0.63, kw=0.30, vec=0.78) — `wildlife.alligators-and-crocodiles#5`
...
**1.** `plants` · *Identification* (hybrid=0.89, kw=1.00, vec=0.84) — `plants.poison-ivy-oak-sumac#0`
...
**2.** `plants` · *If you contact it* (hybrid=0.84, kw=0.96, vec=0.78) — `plants.poison-ivy-oak-sumac#2`
...
**3.** `plants` · *Poison hemlock (Conium maculatum) — historic poison of Socrates* (hybrid=0.75, kw=0.75, vec=0.74) — `plants.dangerous-plants#3`
...
**1.** `plants` · *Burning is dangerous* (hybrid=0.85, kw=0.97, vec=0.80) — `plants.poison-ivy-oak-sumac#3`
  > Never burn poison ivy, oak, or sumac. The urushiol vaporizes into smoke and can be inhaled, causing reactions in the lungs and airways that have killed people. Pull plants by hand only in heavy gloves you can dispose of, and bag the plan…
...
**3.** `plants` · *Treating burns* (hybrid=0.78, kw=0.89, vec=0.73) — `plants.giant-hogweed#3`
...
**3.** `plants` · *Manchineel (Hippomane mancinella) — only in southern Florida and the Caribbean* (hybrid=0.67, kw=0.44, vec=0.76) — `plants.dangerous-plants#4`
...
**3.** `weather` · *After a strike* (hybrid=0.70, kw=0.69, vec=0.70) — `weather.lightning-safety#3`
...
**2.** `weather` · *Warning signs* (hybrid=0.69, kw=0.42, vec=0.81) — `weather.flash-floods#2`
...
**3.** `weather` · *The best shelter* (hybrid=0.66, kw=0.43, vec=0.75) — `weather.lightning-safety#1`
...
**2.** `weather` · *Clothing as the primary defense* (hybrid=0.86, kw=0.99, vec=0.80) — `weather.sun-and-uv#2`
...
**3.** `weather` · *This is an awareness primer, not training* (hybrid=0.72, kw=0.70, vec=0.73) — `weather.avalanche-awareness#0`
...
**1.** `navigation` · *STOP — Stop, Think, Observe, Plan* (hybrid=0.70, kw=0.87, vec=0.63) — `navigation.lost-and-signaling#0`
...
**2.** `trip-basics` · *S — Stop* (hybrid=0.68, kw=0.80, vec=0.63) — `trip-basics.if-you-get-lost#1`
...
**3.** `trip-basics` · *Overview* (hybrid=0.66, kw=0.74, vec=0.63) — `trip-basics.if-you-get-lost#0`
...
**3.** `navigation` · *Declination* (hybrid=0.82, kw=0.81, vec=0.82) — `navigation.map-and-compass#0`
  > A compass needle points to magnetic north; maps are drawn to true north (or grid north, in some military maps). The angle between them is "declination," varies by location, and changes slowly over years. Your topo map's margin states the…
...
**3.** `navigation` · *Taking a bearing from the map* (hybrid=0.63, kw=0.52, vec=0.67) — `navigation.map-and-compass#1`
  > 1. Lay the long edge of the compass baseplate along the line from where you are to where you want to go on the map. 2. Rotate the compass housing until the orienting lines inside it line up with the map's north–south grid lines (north ar…
...
**2.** `navigation` · *What GPS does and doesn't do* (hybrid=0.85, kw=0.90, vec=0.82) — `navigation.gps-basics#0`
...
**3.** `navigation` · *Phone-as-GPS realities* (hybrid=0.79, kw=0.80, vec=0.79) — `navigation.gps-basics#4`
...
**2.** `trip-basics` · *Scout the crossing* (hybrid=0.72, kw=0.91, vec=0.64) — `trip-basics.river-crossings#1`
...
**3.** `weather` · *Putting it together* (hybrid=0.70, kw=0.92, vec=0.61) — `weather.heat-and-cold-microclimates#5`
...
**2.** `weather` · *If you must travel* (hybrid=0.70, kw=0.80, vec=0.66) — `weather.whiteout-conditions#3`
...
**3.** `weather` · *How whiteouts cause trouble* (hybrid=0.70, kw=0.81, vec=0.65) — `weather.whiteout-conditions#1`
...
**1.** `navigation` · *How altimeters work* (hybrid=0.86, kw=0.93, vec=0.84) — `navigation.altimeter-use#0`
...
**3.** `navigation` · *Weather predictor* (hybrid=0.81, kw=0.91, vec=0.77) — `navigation.altimeter-use#3`
...
**2.** `food` · *Meal structure* (hybrid=0.74, kw=0.75, vec=0.73) — `food.meal-planning#2`
...
**3.** `food` · *Cooking temperatures* (hybrid=0.73, kw=0.78, vec=0.71) — `food.food-safety-hygiene#2`
...
**2.** `plants` · *Amatoxin poisoning timeline* (hybrid=0.75, kw=0.85, vec=0.70) — `plants.wild-mushrooms#3`
...
**3.** `water` · *E. coli and other bacteria* (hybrid=0.63, kw=0.40, vec=0.74) — `water.water-borne-illness#2`
  > E. coli, Salmonella, Campylobacter, and Shigella cause vomiting, fever, watery or bloody diarrhea, often within 1–3 days. These are typically from water contaminated by feces (human, livestock, beaver). Boiling, filtration to 0.2 microns…
...
**3.** `knots` · *Common mistakes* (hybrid=0.77, kw=0.60, vec=0.84) — `knots.bowline#2`
...
**3.** `shelter` · *A-frame (symmetric ridgeline)* (hybrid=0.75, kw=0.74, vec=0.76) — `shelter.tarp-configurations#0`
  > The A-frame is the most weather-resistant general-purpose tarp pitch. Run a ridgeline rope between two trees about a body length apart, drape the tarp over it, and stake out the four corners at roughly 45° angles. Tighten guylines so the…
...
**3.** `shelter` · *How to pitch a tent step by step* (hybrid=0.69, kw=0.52, vec=0.76) — `shelter.tent-setup#0`
  > The standard order for setting up most freestanding backpacking tents:  1. **Pick a flat spot** clear of sharp objects, away from widow-makers (dead branches and trees), and not in a low spot where water would pool. 2. **Lay down the foo…
...
**2.** `trip-basics` · *Core categories* (hybrid=0.74, kw=0.80, vec=0.72) — `trip-basics.packing-list#1`
...
**2.** `navigation` · *Stay put or self-rescue* (hybrid=0.60, kw=0.28, vec=0.73) — `navigation.lost-and-signaling#1`
...
**3.** `trip-basics` · *Who to give it to* (hybrid=0.59, kw=0.41, vec=0.67) — `trip-basics.trip-plan#2`
...
**2.** `first-aid` · *Blunt impact / penetrating injury* (hybrid=0.57, kw=0.39, vec=0.64) — `first-aid.eye-injuries#3`
...
**3.** `gear-fixes` · *Multitool or knife* (hybrid=0.52, kw=0.20, vec=0.66) — `gear-fixes.repair-kit#6`
...
**2.** `wildlife` · *Why they matter more than the bite* (hybrid=0.69, kw=0.60, vec=0.72) — `wildlife.ticks-disease-vectors#0`
...
**2.** `plants` · *Why it's dangerous* (hybrid=0.50, kw=0.35, vec=0.56) — `plants.giant-hogweed#0`
...
**3.** `plants` · *Why it kills* (hybrid=0.49, kw=0.34, vec=0.55) — `plants.datura#0`
...
- _domain: top3 = ['wildlife', 'plants', 'plants'], expected 'first-aid'_
