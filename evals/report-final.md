...
**3.** `fire` · *Lighting and feeding* (hybrid=0.80, kw=0.84, vec=0.78) — `fire.building-a-fire#4`
...
**1.** `fire` · *When you'd actually need this* (hybrid=0.86, kw=0.89, vec=0.85) — `fire.primitive-fire-starting#0`
  > Primitive fire-starting — making fire with sticks and friction — is a survival skill, not a routine method. It is hard, time-consuming, and fails far more often than it succeeds even with practice. If you have any modern alternative (a f…

**2.** `fire` · *Honest expectations* (hybrid=0.85, kw=1.00, vec=0.78) — `fire.primitive-fire-starting#7`
  > Primitive fire-starting is a learnable skill, but it takes weeks of practice to become reliable. Many highly-experienced outdoor people cannot make fire from sticks on demand in field conditions. The number-one fire-starting failure for …

**3.** `fire` · *Fire plough* (hybrid=0.78, kw=1.00, vec=0.68) — `fire.primitive-fire-starting#3`
...
**2.** `fire` · *Honest expectations* (hybrid=0.72, kw=0.97, vec=0.61) — `fire.primitive-fire-starting#7`
  > Primitive fire-starting is a learnable skill, but it takes weeks of practice to become reliable. Many highly-experienced outdoor people cannot make fire from sticks on demand in field conditions. The number-one fire-starting failure for …

**3.** `fire` · *Carry purpose-made tinder* (hybrid=0.69, kw=0.57, vec=0.74) — `fire.tinder-and-fire-starting#2`
...
**3.** `fire` · *When you'd actually need this* (hybrid=0.77, kw=0.79, vec=0.76) — `fire.primitive-fire-starting#0`
  > Primitive fire-starting — making fire with sticks and friction — is a survival skill, not a routine method. It is hard, time-consuming, and fails far more often than it succeeds even with practice. If you have any modern alternative (a f…
...
**2.** `fire` · *While it burns* (hybrid=0.67, kw=0.33, vec=0.82) — `fire.fire-safety#1`
...
**3.** `fire` · *Overview* (hybrid=0.66, kw=0.39, vec=0.77) — `fire.leave-no-trace-fire#0`
...
**2.** `fire` · *Use a stove* (hybrid=0.74, kw=0.77, vec=0.73) — `fire.leave-no-trace-fire#1`
...
**3.** `fire` · *Pre-built ready-to-light fires* (hybrid=0.73, kw=0.70, vec=0.74) — `fire.signal-fire#3`
...
**3.** `shelter` · *How to pitch a tent step by step* (hybrid=0.81, kw=0.86, vec=0.78) — `shelter.tent-setup#0`
  > The standard order for setting up most freestanding backpacking tents:  1. **Pick a flat spot** clear of sharp objects, away from widow-makers (dead branches and trees), and not in a low spot where water would pool. 2. **Lay down the foo…
...
**2.** `shelter` · *Site selection for sleeping warm* (hybrid=0.74, kw=0.57, vec=0.81) — `shelter.sleep-system#4`
...
**3.** `weather` · *Wind shadow* (hybrid=0.71, kw=0.54, vec=0.78) — `weather.heat-and-cold-microclimates#4`
...
**2.** `weather` · *If a whiteout starts to develop* (hybrid=0.69, kw=0.75, vec=0.66) — `weather.whiteout-conditions#2`
  > The right move is almost always to stop traveling. Visibility deteriorates faster than it returns. If you have shelter or a known safe terrain feature within easy reach (your tent, a vehicle, a hut, a low-angle ridge in trees), get to it…
...
**1.** `trip-basics` · *Cooking and camp chores* (hybrid=0.77, kw=0.82, vec=0.75) — `trip-basics.group-dynamics#3`
  > Distribute camp work. Common split: one person filters water, another lights stove, third sets up tents, fourth deals with food prep. Cleaning shouldn't fall on the cook. Make this explicit at camp — silent expectations breed resentment.…

**2.** `trip-basics` · *Travel and camp on durable surfaces* (hybrid=0.76, kw=0.79, vec=0.75) — `trip-basics.leave-no-trace#1`
...
**3.** `shelter` · *How to pitch a tent in heavy wind* (hybrid=0.76, kw=0.76, vec=0.75) — `shelter.tent-setup#3`
  > If wind is up, orient the lowest, smallest end of the tent into the wind — usually the foot end. Use every guyline the tent has, double up stakes for the most stressed corners, and tension all guys equally so the fabric doesn't develop w…
...
**3.** `weather` · *Why a 10-foot move changes the temperature* (hybrid=0.75, kw=0.90, vec=0.68) — `weather.heat-and-cold-microclimates#0`
...
**2.** `shelter` · *Insulation under you* (hybrid=0.76, kw=0.71, vec=0.78) — `shelter.hammock-camping#2`
  > The single most-failed lesson of hammock camping: your sleeping bag compresses under you and provides almost no insulation. You need an underquilt (hung under the hammock body, not compressed) or a closed-cell foam pad inside the hammock…

**3.** `fire` · *Safety basics* (hybrid=0.71, kw=0.61, vec=0.75) — `fire.stove-cooking#1`
...
**3.** `shelter` · *Choose a smart spot first* (hybrid=0.73, kw=0.73, vec=0.73) — `shelter.emergency-shelter#0`
...
**3.** `water` · *Sourcing into containers* (hybrid=0.58, kw=0.08, vec=0.79) — `water.water-storage#4`
...
**1.** `water` · *Sources to avoid even after treatment* (hybrid=0.77, kw=0.84, vec=0.74) — `water.unsafe-water-sources#2`
...
**2.** `water` · *Practical priorities* (hybrid=0.71, kw=0.57, vec=0.77) — `water.unsafe-water-sources#4`
...
**3.** `food` · *Eat early, eat often* (hybrid=0.70, kw=0.98, vec=0.59) — `food.calorie-and-nutrition#3`
...
**2.** `water` · *Sources to avoid even after treatment* (hybrid=0.75, kw=0.72, vec=0.76) — `water.unsafe-water-sources#2`
  > Some contamination cannot be removed by filtration or boiling. Avoid these unless absolutely no alternative exists, and even then consider the trade-offs: - **Stagnant water with algae blooms** (green/red scum, foul smell). Cyanobacteria…
...
**1.** `water` · *When to combine methods* (hybrid=0.80, kw=0.86, vec=0.77) — `water.purification-methods#4`
...
**3.** `water` · *Sources that look safer than they are* (hybrid=0.61, kw=0.40, vec=0.70) — `water.unsafe-water-sources#1`
...
**3.** `water` · *Electrolytes matter on long efforts* (hybrid=0.77, kw=0.80, vec=0.76) — `water.dehydration#2`
...
**2.** `food` · *Calories per day* (hybrid=0.79, kw=0.76, vec=0.81) — `food.calorie-and-nutrition#0`
...
**3.** `water` · *Daily intake guidelines* (hybrid=0.66, kw=0.42, vec=0.76) — `water.dehydration#1`
...
**1.** `water` · *Algae and cyanobacteria toxins* (hybrid=0.73, kw=0.90, vec=0.65) — `water.water-borne-illness#4`
...
**3.** `water` · *Sources to avoid even after treatment* (hybrid=0.70, kw=0.84, vec=0.64) — `water.unsafe-water-sources#2`
  > Some contamination cannot be removed by filtration or boiling. Avoid these unless absolutely no alternative exists, and even then consider the trade-offs: - **Stagnant water with algae blooms** (green/red scum, foul smell). Cyanobacteria…
...
**3.** `water` · *Snow and ice* (hybrid=0.79, kw=0.89, vec=0.75) — `water.finding-water#3`
...
**2.** `water` · *Sources that look safer than they are* (hybrid=0.63, kw=0.56, vec=0.65) — `water.unsafe-water-sources#1`
  > - **High-elevation alpine streams**: usually clean of human pathogens but can carry giardia from upstream wildlife. Treat anyway. - **Snowmelt**: pure-looking but may carry whatever was on or in the snowpack. Treat. - **"Pristine"-lookin…

**3.** `water` · *Practical priorities* (hybrid=0.61, kw=0.37, vec=0.71) — `water.unsafe-water-sources#4`
  > 1. Best: moving water from springs or mid-stream, in remote drainages, away from grazing, mining, and downstream of campsites. 2. Acceptable: most alpine streams and lakes after proper treatment. 3. Last resort: cloudy or silty water — p…
...
**1.** `first-aid` · *Recognize life-threatening bleeding* (hybrid=0.80, kw=0.82, vec=0.79) — `first-aid.bleeding-control#0`
...
**2.** `first-aid` · *When to evacuate* (hybrid=0.74, kw=0.88, vec=0.68) — `first-aid.nosebleed#3`
...
**3.** `first-aid` · *What to avoid* (hybrid=0.72, kw=0.60, vec=0.77) — `first-aid.bleeding-control#3`
...
**3.** `trip-basics` · *Minimize campfire impacts* (hybrid=0.73, kw=0.71, vec=0.73) — `trip-basics.leave-no-trace#3`
...
**1.** `first-aid` · *Recognize the signs* (hybrid=0.85, kw=0.92, vec=0.82) — `first-aid.hypothermia#0`
...
**2.** `first-aid` · *Rewarm gently* (hybrid=0.71, kw=0.61, vec=0.75) — `first-aid.hypothermia#2`
...
**3.** `first-aid` · *Severe hypothermia is a medical emergency* (hybrid=0.69, kw=0.44, vec=0.80) — `first-aid.hypothermia#3`
...
**3.** `first-aid` · *Treat heat exhaustion* (hybrid=0.70, kw=0.85, vec=0.64) — `first-aid.heat-illness#1`
...
**1.** `first-aid` · *RICE for sprains* (hybrid=0.76, kw=0.81, vec=0.74) — `first-aid.sprains-fractures#1`
...
**2.** `knots` · *How to tie it (two-loop method, on the bight)* (hybrid=0.73, kw=1.00, vec=0.61) — `knots.clove-hitch#1`
...
**3.** `first-aid` · *What NOT to do* (hybrid=0.68, kw=0.98, vec=0.55) — `first-aid.tick-removal#3`
...
- _top result hit keywords: ['RICE', 'ice', 'compression', 'elevation']_
...
**1.** `wildlife` · *Get away from the snake* (hybrid=0.79, kw=0.94, vec=0.73) — `wildlife.venomous-snake#1`
...
**3.** `wildlife` · *What to do if you see a snake* (hybrid=0.78, kw=0.86, vec=0.74) — `wildlife.snakes#2`
...
**1.** `first-aid` · *What NOT to do* (hybrid=0.80, kw=0.87, vec=0.77) — `first-aid.tick-removal#3`
  > - Do not use petroleum jelly, nail polish, or other substances to "smother" the tick. - Do not apply heat (a match, a hot needle) to the tick. - Do not twist, jerk, or wiggle the tick out. - Do not crush the tick body during removal.  Al…

**2.** `wildlife` · *Removing an attached tick* (hybrid=0.77, kw=0.82, vec=0.74) — `wildlife.ticks-disease-vectors#3`
...
**3.** `first-aid` · *How to remove a tick (CDC method)* (hybrid=0.76, kw=0.76, vec=0.76) — `first-aid.tick-removal#2`
  > 1. **Grasp the tick** as close to the skin as possible. Get the mouthparts, not the body. 2. **Pull straight up** with steady, even pressure. Do not twist. Do not jerk. Do not squeeze the body — squeezing pumps the tick's gut contents (a…
...
**2.** `trip-basics` · *Common pre-trip mistakes* (hybrid=0.80, kw=0.99, vec=0.71) — `trip-basics.conditioning#2`
...
**3.** `gear-fixes` · *Hot spots and blisters mid-trip* (hybrid=0.79, kw=0.68, vec=0.84) — `gear-fixes.footwear-emergencies#3`
...
**1.** `first-aid` · *Acute Mountain Sickness (AMS)* (hybrid=0.84, kw=0.98, vec=0.78) — `first-aid.altitude-illness#0`
...
**2.** `first-aid` · *Field treatment — deep frostbite* (hybrid=0.68, kw=0.38, vec=0.81) — `first-aid.frostbite#2`
...
**1.** `wildlife` · *Anaphylaxis after a sting* (hybrid=0.79, kw=0.98, vec=0.70) — `wildlife.bees-wasps-hornets#5`
...
**2.** `first-aid` · *Anaphylaxis (severe allergic reaction)* (hybrid=0.72, kw=0.81, vec=0.68) — `first-aid.bites-stings#3`
...
**2.** `first-aid` · *Aftermath* (hybrid=0.67, kw=0.45, vec=0.77) — `first-aid.choking#4`
...
**3.** `first-aid` · *Adult / older child: abdominal thrusts (Heimlich)* (hybrid=0.66, kw=0.35, vec=0.79) — `first-aid.choking#1`
...
**2.** `first-aid` · *This is not training* (hybrid=0.76, kw=0.69, vec=0.79) — `first-aid.cpr-basics#0`
...
**3.** `first-aid` · *Once you have the person out* (hybrid=0.68, kw=0.53, vec=0.74) — `first-aid.drowning#2`
...
**2.** `first-aid` · *Recognize life-threatening bleeding* (hybrid=0.69, kw=0.43, vec=0.80) — `first-aid.bleeding-control#0`
  > Life-threatening bleeding is bleeding that is spurting, pooling on the ground, soaking through clothing, or that won't stop with simple pressure. If the person becomes pale, cold, sweaty, confused, or weak, treat as life-threatening rega…

**3.** `first-aid` · *Stopping a typical nosebleed* (hybrid=0.65, kw=0.43, vec=0.75) — `first-aid.nosebleed#1`
...
**2.** `first-aid` · *Overview* (hybrid=0.60, kw=0.36, vec=0.71) — `first-aid.head-injury#0`
...
**3.** `navigation` · *Aim-off* (hybrid=0.59, kw=0.66, vec=0.56) — `navigation.handrails-and-aim-off#2`
...
**2.** `first-aid` · *Telling sprains from fractures in the field* (hybrid=0.66, kw=0.69, vec=0.65) — `first-aid.sprains-fractures#0`
...
**3.** `first-aid` · *Splinting a suspected fracture* (hybrid=0.65, kw=0.59, vec=0.68) — `first-aid.sprains-fractures#2`
  > Splint the joint above and the joint below the suspected break to immobilize it. Use trekking poles, sleeping pad strips, sticks, or even a foam roll — whatever is rigid. Pad bony spots with cloth so the splint doesn't cause its own pres…
...
**2.** `water` · *E. coli and other bacteria* (hybrid=0.72, kw=0.72, vec=0.73) — `water.water-borne-illness#2`
...
**1.** `first-aid` · *Once you have the person out* (hybrid=0.79, kw=0.90, vec=0.74) — `first-aid.drowning#2`
  > Position them on their back. Tilt the head back, lift the chin, look-listen-feel for breathing for no more than 10 seconds. If not breathing or only gasping, start CPR immediately. The priority for drowning victims is rescue breaths — gi…

**2.** `first-aid` · *Position and protect* (hybrid=0.77, kw=1.00, vec=0.68) — `first-aid.anaphylaxis#2`
...
**3.** `first-aid` · *Check, call, compress* (hybrid=0.73, kw=0.53, vec=0.82) — `first-aid.cpr-basics#1`
...
**1.** `wildlife` · *If you see a bear at a distance* (hybrid=0.87, kw=0.95, vec=0.84) — `wildlife.bears#2`
...
**2.** `wildlife` · *If you see a bear at a distance* (hybrid=0.87, kw=0.93, vec=0.84) — `wildlife.bear-encounter#0`
...
**3.** `wildlife` · *If a moose charges* (hybrid=0.76, kw=0.83, vec=0.74) — `wildlife.moose-and-large-ungulates#2`
...
**1.** `wildlife` · *If you see a bear at a distance* (hybrid=0.75, kw=0.56, vec=0.83) — `wildlife.bears#2`
  > Stop and assess. If the bear hasn't seen you, back away quietly the way you came and detour widely. If the bear has seen you, identify yourself as human: speak calmly, wave your arms slowly. Do not run; running triggers chase instinct in…

**2.** `wildlife` · *When to deploy* (hybrid=0.75, kw=0.62, vec=0.80) — `wildlife.bear-spray#3`
  > Deploy when a bear is charging, approaching aggressively, or has made contact. Do not spray a bear that is calmly grazing or moving away — back away yourself.

**3.** `wildlife` · *Porcupines* (hybrid=0.73, kw=1.00, vec=0.61) — `wildlife.raccoons-and-rodents#3`
...
- _top result hit keywords: ['group']_
...
**2.** `wildlife` · *If a moose charges* (hybrid=0.76, kw=0.58, vec=0.83) — `wildlife.moose-and-large-ungulates#2`
  > Run. This is the exception — unlike bears and mountain lions, you run from moose. Get behind a substantial tree (trees a moose can't easily knock down) or other solid obstacle. Moose charge to dislodge a threat, not to kill, and will usu…

**3.** `wildlife` · *Reducing the risk* (hybrid=0.76, kw=0.57, vec=0.83) — `wildlife.moose#5`
...
**2.** `wildlife` · *What to do if you see a snake* (hybrid=0.65, kw=0.40, vec=0.75) — `wildlife.snakes#2`
  > Stop. Identify where it is and let it move away on its own — most snakes will leave if not threatened. Detour widely; give it at least 5–6 feet (2 m) of clearance. Do not try to handle it, kill it, or photograph it up close. About half o…

**3.** `wildlife` · *Avoiding snakes in the first place* (hybrid=0.64, kw=0.39, vec=0.75) — `wildlife.snakes#0`
  > Watch where you put your hands and feet — especially when stepping over logs, reaching into brush, or climbing around rocks. Snakes use the same trails and warm rocks people do. Hike during cooler parts of the day in hot climates; snakes…
...
**2.** `wildlife` · *Coyotes* (hybrid=0.77, kw=0.74, vec=0.78) — `wildlife.coyotes-and-wolves#0`
...
**3.** `wildlife` · *Foxes* (hybrid=0.74, kw=0.77, vec=0.73) — `wildlife.coyotes-and-wolves#2`
...
**2.** `wildlife` · *In the water* (hybrid=0.64, kw=0.41, vec=0.74) — `wildlife.alligators-and-crocodiles#4`
...
**3.** `wildlife` · *If attacked* (hybrid=0.64, kw=0.31, vec=0.78) — `wildlife.alligators-and-crocodiles#5`
...
**2.** `wildlife` · *Removing a stinger* (hybrid=0.67, kw=0.35, vec=0.80) — `wildlife.bees-wasps-hornets#3`
...
**3.** `wildlife` · *US scorpions* (hybrid=0.60, kw=0.21, vec=0.77) — `wildlife.spiders-and-scorpions#1`
...
**2.** `plants` · *Poison hemlock identification* (hybrid=0.80, kw=0.99, vec=0.73) — `plants.poison-hemlock#1`
...
**3.** `plants` · *Poison hemlock (Conium maculatum) — historic poison of Socrates* (hybrid=0.74, kw=0.73, vec=0.74) — `plants.dangerous-plants#3`
...
**1.** `plants` · *Burning is dangerous* (hybrid=0.85, kw=0.96, vec=0.80) — `plants.poison-ivy-oak-sumac#3`
  > Never burn poison ivy, oak, or sumac. The urushiol vaporizes into smoke and can be inhaled, causing reactions in the lungs and airways that have killed people. Pull plants by hand only in heavy gloves you can dispose of, and bag the plan…

**2.** `plants` · *Overview* (hybrid=0.84, kw=1.00, vec=0.77) — `plants.dangerous-plants#0`
...
**3.** `plants` · *Don't burn oleander* (hybrid=0.81, kw=0.98, vec=0.74) — `plants.oleander#3`
...
**3.** `plants` · *Manchineel (Hippomane mancinella) — only in southern Florida and the Caribbean* (hybrid=0.66, kw=0.43, vec=0.76) — `plants.dangerous-plants#4`
...
**3.** `weather` · *After a strike* (hybrid=0.69, kw=0.68, vec=0.70) — `weather.lightning-safety#3`
...
**2.** `weather` · *Warning signs* (hybrid=0.69, kw=0.39, vec=0.81) — `weather.flash-floods#2`
...
**3.** `weather` · *The best shelter* (hybrid=0.65, kw=0.40, vec=0.75) — `weather.lightning-safety#1`
...
**2.** `first-aid` · *If you must move them* (hybrid=0.58, kw=0.48, vec=0.62) — `first-aid.spinal-injury#2`
...
**3.** `weather` · *Get the warnings before they arrive* (hybrid=0.57, kw=0.33, vec=0.68) — `weather.tornado-and-severe#4`
...
**1.** `weather` · *Why they kill so many people* (hybrid=0.81, kw=0.77, vec=0.82) — `weather.flash-floods#0`
...
**2.** `weather` · *Action if caught* (hybrid=0.76, kw=0.72, vec=0.77) — `weather.flash-floods#3`
...
**1.** `weather` · *Recognize the threat* (hybrid=0.84, kw=0.93, vec=0.80) — `weather.tornado-and-severe#0`
...
**2.** `weather` · *Best shelter outdoors* (hybrid=0.80, kw=0.86, vec=0.78) — `weather.tornado-and-severe#1`
...
**2.** `weather` · *Prevention* (hybrid=0.80, kw=0.79, vec=0.80) — `weather.whiteout-conditions#5`
...
**3.** `weather` · *Plan* (hybrid=0.78, kw=0.74, vec=0.80) — `weather.sun-and-uv#6`
...
**3.** `weather` · *This is an awareness primer, not training* (hybrid=0.72, kw=0.71, vec=0.73) — `weather.avalanche-awareness#0`
...
**1.** `navigation` · *STOP — Stop, Think, Observe, Plan* (hybrid=0.69, kw=0.84, vec=0.63) — `navigation.lost-and-signaling#0`
...
**2.** `trip-basics` · *S — Stop* (hybrid=0.67, kw=0.78, vec=0.63) — `trip-basics.if-you-get-lost#1`
...
**3.** `trip-basics` · *Overview* (hybrid=0.66, kw=0.72, vec=0.63) — `trip-basics.if-you-get-lost#0`
  > The moment you realize you don't know where you are, the worst thing you can do is keep walking in hopes of "finding the trail." Wandering when lost is the single biggest cause of survival situations becoming fatal. There is a simple acr…
...
**1.** `navigation` · *Taking a bearing from the map* (hybrid=0.91, kw=0.94, vec=0.89) — `navigation.map-and-compass#1`
...
**2.** `navigation` · *Taking a bearing from the map* (hybrid=0.57, kw=0.49, vec=0.61) — `navigation.map-and-compass#1`
  > 1. Lay the long edge of the compass baseplate along the line from where you are to where you want to go on the map. 2. Rotate the compass housing until the orienting lines inside it line up with the map's north–south grid lines (north ar…
...
**2.** `navigation` · *What GPS does and doesn't do* (hybrid=0.84, kw=0.90, vec=0.82) — `navigation.gps-basics#0`
...
**1.** `navigation` · *Pace counting* (hybrid=0.80, kw=0.95, vec=0.74) — `navigation.pace-and-time#1`
...
**3.** `wildlife` · *Overview* (hybrid=0.79, kw=0.74, vec=0.81) — `wildlife.food-storage#0`
...
**2.** `food` · *Calorie targets by trip type* (hybrid=0.78, kw=0.88, vec=0.74) — `food.meal-planning#0`
...
**3.** `food` · *Weight per day targets* (hybrid=0.59, kw=0.34, vec=0.69) — `food.meal-planning#1`
...
**2.** `food` · *Meal structure* (hybrid=0.74, kw=0.74, vec=0.73) — `food.meal-planning#2`
...
**3.** `trip-basics` · *Weight discipline* (hybrid=0.73, kw=0.67, vec=0.76) — `trip-basics.packing-list#5`
...
**2.** `plants` · *Amatoxin poisoning timeline* (hybrid=0.74, kw=0.81, vec=0.70) — `plants.wild-mushrooms#3`
...
**3.** `plants` · *What to do* (hybrid=0.73, kw=0.73, vec=0.73) — `plants.pokeweed#4`
  > Call Poison Control. Evacuate to medical care for any significant ingestion, especially in children. Treatment is supportive: IV fluids to replace what's lost in vomiting and diarrhea, antiemetics, electrolyte monitoring. There is no ant…
...
**1.** `knots` · *What it does* (hybrid=0.90, kw=0.95, vec=0.88) — `knots.bowline#0`
...
**2.** `knots` · *When not to use it* (hybrid=0.90, kw=1.00, vec=0.85) — `knots.bowline#3`
...
**3.** `knots` · *Common mistakes* (hybrid=0.76, kw=0.60, vec=0.84) — `knots.bowline#2`
...
- _top result hit keywords: ['loop']_
...
**3.** `knots` · *Best uses* (hybrid=0.80, kw=0.81, vec=0.80) — `knots.square-knot#3`
...
**2.** `knots` · *When to use what* (hybrid=0.81, kw=0.74, vec=0.84) — `knots.two-half-hitches#3`
  > - Tying a rope to a tree or post: two half hitches, or a bowline if you want a fixed loop. - Joining two ropes for tension (clothesline, ridgeline extension): sheet bend. - Joining two ropes for life-safety: not in this pack — take a cla…
...
**3.** `shelter` · *A-frame (symmetric ridgeline)* (hybrid=0.75, kw=0.73, vec=0.76) — `shelter.tarp-configurations#0`
  > The A-frame is the most weather-resistant general-purpose tarp pitch. Run a ridgeline rope between two trees about a body length apart, drape the tarp over it, and stake out the four corners at roughly 45° angles. Tighten guylines so the…
...
**1.** `knots` · *Recognize a correct square knot* (hybrid=0.87, kw=0.95, vec=0.83) — `knots.square-knot#2`
...
**3.** `knots` · *Square knot (reef knot) — and what it isn't* (hybrid=0.82, kw=0.88, vec=0.80) — `knots.two-half-hitches#2`
  > A square knot binds the ends of two ropes of equal diameter around something — most commonly to tie off a bundle, secure a bandage, or lash gear. To tie: right over left, then left over right (the mantra). The square knot is **not** safe…
...
**3.** `gear-fixes` · *Sole separation* (hybrid=0.79, kw=0.76, vec=0.81) — `gear-fixes.footwear-emergencies#0`
...
**2.** `gear-fixes` · *Torn pack body* (hybrid=0.76, kw=0.70, vec=0.79) — `gear-fixes.pack-strap-repair#3`
...
**2.** `gear-fixes` · *Broken tent pole* (hybrid=0.87, kw=0.83, vec=0.88) — `gear-fixes.field-repairs#1`
...
**3.** `shelter` · *How to pitch a tent step by step* (hybrid=0.69, kw=0.53, vec=0.76) — `shelter.tent-setup#0`
  > The standard order for setting up most freestanding backpacking tents:  1. **Pick a flat spot** clear of sharp objects, away from widow-makers (dead branches and trees), and not in a low spot where water would pool. 2. **Lay down the foo…
...
**3.** `shelter` · *The pad is half the system* (hybrid=0.65, kw=0.52, vec=0.70) — `shelter.sleep-system#0`
...
**2.** `gear-fixes` · *Frame damage* (hybrid=0.83, kw=0.98, vec=0.77) — `gear-fixes.pack-strap-repair#4`
  > Internal-frame packs sometimes break their aluminum stays. A bent stay can sometimes be straightened by hand. A broken stay needs a splint — a stick of similar length lashed to the broken stay with cord, with the broken pieces aligned. T…
...
**2.** `trip-basics` · *Core categories* (hybrid=0.74, kw=0.81, vec=0.72) — `trip-basics.packing-list#1`
...
**3.** `trip-basics` · *Bring them every time* (hybrid=0.74, kw=0.59, vec=0.80) — `trip-basics.ten-essentials#3`
...
**2.** `trip-basics` · *Plan ahead and prepare* (hybrid=0.68, kw=0.91, vec=0.59) — `trip-basics.leave-no-trace#0`
...
**2.** `navigation` · *Stay put or self-rescue* (hybrid=0.60, kw=0.29, vec=0.73) — `navigation.lost-and-signaling#1`
...
**3.** `trip-basics` · *Who to give it to* (hybrid=0.59, kw=0.42, vec=0.67) — `trip-basics.trip-plan#2`
  > Pick someone who will actually be near a phone, will remember to follow up, and is willing to make the call. A spouse, parent, or close friend works. A coworker who travels constantly is a bad choice.
...
**2.** `trip-basics` · *Simple training plan* (hybrid=0.79, kw=0.65, vec=0.86) — `trip-basics.conditioning#1`
...
**3.** `trip-basics` · *Common pre-trip mistakes* (hybrid=0.79, kw=0.77, vec=0.80) — `trip-basics.conditioning#2`
  > "I'll get into shape on the trail" — no, the trail will get you into shape if you survive the first 3 days. Old injuries flare up under loaded hiking; address them before the trip. New boots not broken in cause blister disasters; walk in…
...
**2.** `trip-basics` · *Use a written checklist* (hybrid=0.83, kw=0.89, vec=0.80) — `trip-basics.packing-list#0`
...
**2.** `trip-basics` · *Set a turnaround time before you leave* (hybrid=0.77, kw=0.84, vec=0.74) — `trip-basics.when-to-turn-back#1`
...
**3.** `fire` · *The three sizes* (hybrid=0.61, kw=0.47, vec=0.67) — `fire.tinder-and-fire-starting#1`
...
**2.** `first-aid` · *Blunt impact / penetrating injury* (hybrid=0.57, kw=0.40, vec=0.64) — `first-aid.eye-injuries#3`
...
**3.** `trip-basics` · *Decide whether to cross* (hybrid=0.70, kw=0.61, vec=0.74) — `trip-basics.river-crossings#2`
  > Don't cross water above your thighs in significant current. Don't cross moving water that's brown, churning, or carrying debris — those signal a flood pulse, often from upstream rain. Don't cross if there's no clear exit on the far side,…
...
**2.** `first-aid` · *Prevent* (hybrid=0.68, kw=0.59, vec=0.72) — `first-aid.blisters#1`
  > - **Break in new boots** at home: short walks, longer walks, day hikes — before any multi-day trip. - **Wear two socks**: a thin liner sock under a thicker outer sock. The friction happens between the two socks, not between sock and skin…

**3.** `fire` · *Lighters* (hybrid=0.63, kw=0.77, vec=0.58) — `fire.ignition-sources#1`
...
**1.** `wildlife` · *Prevention: clothing and chemistry* (hybrid=0.76, kw=1.00, vec=0.66) — `wildlife.ticks-disease-vectors#1`
...
**2.** `wildlife` · *Why they matter more than the bite* (hybrid=0.69, kw=0.61, vec=0.72) — `wildlife.ticks-disease-vectors#0`
...
**3.** `wildlife` · *Mosquito-specific protection* (hybrid=0.66, kw=0.62, vec=0.68) — `wildlife.ticks-disease-vectors#5`
  > Cover up at dawn and dusk when most species are most active. Sleep under a mesh head net or in a tent with sealed mesh, not a fly-only tarp. Sleeping pad and bag treated with permethrin discourages mosquitoes through the night. In areas …

- _top result hit keywords: ['permethrin', 'DEET']_
...
**3.** `first-aid` · *Call-for-rescue triggers* (hybrid=0.61, kw=0.79, vec=0.54) — `first-aid.evacuation-decision#3`
  > Any of the following triggers an immediate call/PLB/satellite SOS if you can: unconsciousness or altered mental status, breathing difficulty unrelated to exertion, chest pain, suspected stroke or heart attack, suspected spinal injury, un…
