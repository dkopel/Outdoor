...
**1.** `fire` · *Honest expectations* (hybrid=0.85, kw=1.00, vec=0.78) — `fire.primitive-fire-starting#7`
  > Primitive fire-starting is a learnable skill, but it takes weeks of practice to become reliable. Many highly-experienced outdoor people cannot make fire from sticks on demand in field conditions. The number-one fire-starting failure for …

**2.** `fire` · *When you'd actually need this* (hybrid=0.84, kw=0.80, vec=0.86) — `fire.primitive-fire-starting#0`
  > Primitive fire-starting — making fire with sticks and friction — is a survival skill, not a routine method. It is hard, time-consuming, and fails far more often than it succeeds even with practice. If you have any modern alternative (a f…

**3.** `fire` · *Lighting and feeding* (hybrid=0.80, kw=0.85, vec=0.78) — `fire.building-a-fire#4`
...
- _top result hit keywords: ['tinder']_
...
**1.** `fire` · *How to build a campfire step by step* (hybrid=0.80, kw=0.91, vec=0.75) — `fire.building-a-fire#0`
  > 1. **Pick a safe spot** — existing fire ring if one is there. Clear a 10-foot radius down to bare mineral soil. 2. **Gather all three fuel sizes before you light a single match** — tinder (small dry plant fibers), kindling (pencil-thin t…

**2.** `fire` · *The teepee lay* (hybrid=0.77, kw=1.00, vec=0.67) — `fire.building-a-fire#2`
...
**3.** `fire` · *The lighting sequence* (hybrid=0.74, kw=0.85, vec=0.69) — `fire.tinder-and-fire-starting#4`
  > 1. Build the lay (teepee or log cabin) **with kindling pre-stacked** around the tinder bird's-nest. 2. Strike the spark or flame into the tinder. Once it catches, blow gently and steadily on the base of the flame to feed oxygen. 3. As th…

- _top result hit keywords: ['teepee', 'lay', 'tinder']_
...
**1.** `fire` · *Honest expectations* (hybrid=0.86, kw=1.00, vec=0.79) — `fire.primitive-fire-starting#7`
  > Primitive fire-starting is a learnable skill, but it takes weeks of practice to become reliable. Many highly-experienced outdoor people cannot make fire from sticks on demand in field conditions. The number-one fire-starting failure for …

**2.** `fire` · *When you'd actually need this* (hybrid=0.77, kw=0.75, vec=0.78) — `fire.primitive-fire-starting#0`
  > Primitive fire-starting — making fire with sticks and friction — is a survival skill, not a routine method. It is hard, time-consuming, and fails far more often than it succeeds even with practice. If you have any modern alternative (a f…

**3.** `fire` · *Modern non-flame alternatives* (hybrid=0.77, kw=0.79, vec=0.76) — `fire.primitive-fire-starting#5`
  > **Magnifying lens.** A small magnifying glass, the lens from binoculars or eyeglasses, or even a clear water-filled bottle can focus sunlight to ignite dry tinder. Only works in direct sun. Cheap and small enough to keep in a kit.  **Bat…

- _top result hit keywords: ['ferro', 'primitive', 'sticks']_
...
**1.** `fire` · *When you'd actually need this* (hybrid=0.86, kw=0.88, vec=0.85) — `fire.primitive-fire-starting#0`
  > Primitive fire-starting — making fire with sticks and friction — is a survival skill, not a routine method. It is hard, time-consuming, and fails far more often than it succeeds even with practice. If you have any modern alternative (a f…

**2.** `fire` · *Honest expectations* (hybrid=0.85, kw=1.00, vec=0.78) — `fire.primitive-fire-starting#7`
  > Primitive fire-starting is a learnable skill, but it takes weeks of practice to become reliable. Many highly-experienced outdoor people cannot make fire from sticks on demand in field conditions. The number-one fire-starting failure for …

**3.** `fire` · *Fire plough* (hybrid=0.78, kw=0.99, vec=0.68) — `fire.primitive-fire-starting#3`
  > A simpler method that works for some wood types. Cut a groove into a piece of softwood. Plow a hardwood stick back and forth in the groove, pressing down. Friction creates hot dust at the end of the stroke. Smaller chance of success than…

- _top result hit keywords: ['friction']_
...
**2.** `fire` · *Lighting in worst conditions* (hybrid=0.79, kw=0.78, vec=0.80) — `fire.ignition-sources#5`
...
**3.** `fire` · *Be patient and feed it* (hybrid=0.75, kw=0.67, vec=0.79) — `fire.wet-conditions#3`
...
**2.** `fire` · *Honest expectations* (hybrid=0.72, kw=0.96, vec=0.61) — `fire.primitive-fire-starting#7`
  > Primitive fire-starting is a learnable skill, but it takes weeks of practice to become reliable. Many highly-experienced outdoor people cannot make fire from sticks on demand in field conditions. The number-one fire-starting failure for …

**3.** `fire` · *Carry purpose-made tinder* (hybrid=0.69, kw=0.56, vec=0.74) — `fire.tinder-and-fire-starting#2`
...
**2.** `fire` · *Modern non-flame alternatives* (hybrid=0.80, kw=0.84, vec=0.78) — `fire.primitive-fire-starting#5`
  > **Magnifying lens.** A small magnifying glass, the lens from binoculars or eyeglasses, or even a clear water-filled bottle can focus sunlight to ignite dry tinder. Only works in direct sun. Cheap and small enough to keep in a kit.  **Bat…

**3.** `fire` · *When you'd actually need this* (hybrid=0.76, kw=0.78, vec=0.76) — `fire.primitive-fire-starting#0`
  > Primitive fire-starting — making fire with sticks and friction — is a survival skill, not a routine method. It is hard, time-consuming, and fails far more often than it succeeds even with practice. If you have any modern alternative (a f…
...
**2.** `fire` · *While it burns* (hybrid=0.68, kw=0.35, vec=0.82) — `fire.fire-safety#1`
...
**3.** `fire` · *Overview* (hybrid=0.66, kw=0.40, vec=0.77) — `fire.leave-no-trace-fire#0`
...
**1.** `fire` · *The teepee lay* (hybrid=0.79, kw=1.00, vec=0.70) — `fire.building-a-fire#2`
  > Stack tinder loosely on a base of small sticks so it doesn't sit in cold dirt or wet ground. Build a teepee of kindling around the tinder, leaving an opening on the windward side to feed in flame and oxygen. Light the tinder through the …

**2.** `fire` · *Use a stove* (hybrid=0.75, kw=0.81, vec=0.73) — `fire.leave-no-trace-fire#1`
...
**3.** `fire` · *Pre-built ready-to-light fires* (hybrid=0.74, kw=0.74, vec=0.74) — `fire.signal-fire#3`
...
**3.** `trip-basics` · *Cooking and camp chores* (hybrid=0.71, kw=0.89, vec=0.63) — `trip-basics.group-dynamics#3`
...
**2.** `fire` · *When to light* (hybrid=0.81, kw=0.83, vec=0.80) — `fire.signal-fire#4`
...
**3.** `fire` · *The international distress pattern* (hybrid=0.78, kw=0.73, vec=0.80) — `fire.signal-fire#0`
...
**2.** `fire` · *Carry purpose-made tinder* (hybrid=0.70, kw=0.80, vec=0.66) — `fire.tinder-and-fire-starting#2`
  > Always carry one or two reliable tinder sources that don't depend on field conditions: - **Petroleum-jelly cotton balls** in a small ziplock — burn 60+ seconds, light from one spark, cheap. - **Fatwood / pitch wood** — resinous pine hear…

**3.** `fire` · *Adapting to wet conditions* (hybrid=0.70, kw=0.58, vec=0.74) — `fire.tinder-and-fire-starting#5`
...
**1.** `shelter` · *How to pitch a tent step by step* (hybrid=0.90, kw=1.00, vec=0.86) — `shelter.tent-setup#0`
  > The standard order for setting up most freestanding backpacking tents:  1. **Pick a flat spot** clear of sharp objects, away from widow-makers (dead branches and trees), and not in a low spot where water would pool. 2. **Lay down the foo…

**2.** `food` · *The bear triangle* (hybrid=0.79, kw=0.84, vec=0.77) — `food.cooking-in-bear-country#1`
...
**3.** `weather` · *If a whiteout starts to develop* (hybrid=0.73, kw=0.85, vec=0.68) — `weather.whiteout-conditions#2`
...
- _top result hit keywords: ['stake', 'fly', 'pole']_
...
**1.** `shelter` · *How to pitch a tent in heavy wind* (hybrid=0.87, kw=1.00, vec=0.81) — `shelter.tent-setup#3`
...
**2.** `shelter` · *Taking the tent down* (hybrid=0.83, kw=0.99, vec=0.77) — `shelter.tent-setup#4`
...
**3.** `shelter` · *How to pitch a tent step by step* (hybrid=0.80, kw=0.86, vec=0.78) — `shelter.tent-setup#0`
  > The standard order for setting up most freestanding backpacking tents:  1. **Pick a flat spot** clear of sharp objects, away from widow-makers (dead branches and trees), and not in a low spot where water would pool. 2. **Lay down the foo…
...
**1.** `shelter` · *How to pitch a tent in heavy wind* (hybrid=0.92, kw=1.00, vec=0.88) — `shelter.tent-setup#3`
  > If wind is up, orient the lowest, smallest end of the tent into the wind — usually the foot end. Use every guyline the tent has, double up stakes for the most stressed corners, and tension all guys equally so the fabric doesn't develop w…

**2.** `shelter` · *Site selection for sleeping warm* (hybrid=0.74, kw=0.59, vec=0.81) — `shelter.sleep-system#4`
...
**3.** `weather` · *Wind shadow* (hybrid=0.72, kw=0.57, vec=0.78) — `weather.heat-and-cold-microclimates#4`
...
- _top result hit keywords: ['guyline', 'stake']_
...
**2.** `weather` · *If a whiteout starts to develop* (hybrid=0.68, kw=0.74, vec=0.66) — `weather.whiteout-conditions#2`
  > The right move is almost always to stop traveling. Visibility deteriorates faster than it returns. If you have shelter or a known safe terrain feature within easy reach (your tent, a vehicle, a hut, a low-angle ridge in trees), get to it…

**3.** `shelter` · *A-frame (symmetric ridgeline)* (hybrid=0.68, kw=0.38, vec=0.80) — `shelter.tarp-configurations#0`
...
**1.** `trip-basics` · *Cooking and camp chores* (hybrid=0.77, kw=0.83, vec=0.75) — `trip-basics.group-dynamics#3`
  > Distribute camp work. Common split: one person filters water, another lights stove, third sets up tents, fourth deals with food prep. Cleaning shouldn't fall on the cook. Make this explicit at camp — silent expectations breed resentment.…

**2.** `trip-basics` · *Travel and camp on durable surfaces* (hybrid=0.77, kw=0.79, vec=0.75) — `trip-basics.leave-no-trace#1`
...
**3.** `shelter` · *Insulation under you* (hybrid=0.76, kw=1.00, vec=0.66) — `shelter.hammock-camping#2`
...
**2.** `shelter` · *What a bivy is and isn't* (hybrid=0.78, kw=0.94, vec=0.71) — `shelter.bivy-sack#0`
...
**3.** `shelter` · *How to pitch a tent in heavy wind* (hybrid=0.76, kw=0.77, vec=0.75) — `shelter.tent-setup#3`
  > If wind is up, orient the lowest, smallest end of the tent into the wind — usually the foot end. Use every guyline the tent has, double up stakes for the most stressed corners, and tension all guys equally so the fabric doesn't develop w…
...
**1.** `shelter` · *Eating and drinking for warmth* (hybrid=0.87, kw=1.00, vec=0.81) — `shelter.sleep-system#5`
...
**2.** `shelter` · *Layering inside a bag* (hybrid=0.82, kw=0.94, vec=0.77) — `shelter.sleep-system#3`
...
**3.** `weather` · *Why a 10-foot move changes the temperature* (hybrid=0.74, kw=0.90, vec=0.68) — `weather.heat-and-cold-microclimates#0`
  > A campsite at the bottom of a valley can be 15–20°F colder than a bench 50 feet above it on a calm clear night — cold dense air sinks and pools. South-facing slopes get more sun and stay warmer in winter; north-facing slopes hold snow lo…
...
**2.** `shelter` · *Insulation under you* (hybrid=0.76, kw=0.70, vec=0.78) — `shelter.hammock-camping#2`
  > The single most-failed lesson of hammock camping: your sleeping bag compresses under you and provides almost no insulation. You need an underquilt (hung under the hammock body, not compressed) or a closed-cell foam pad inside the hammock…

**3.** `fire` · *Safety basics* (hybrid=0.71, kw=0.62, vec=0.75) — `fire.stove-cooking#1`
...
**2.** `weather` · *If a whiteout starts to develop* (hybrid=0.73, kw=0.70, vec=0.75) — `weather.whiteout-conditions#2`
  > The right move is almost always to stop traveling. Visibility deteriorates faster than it returns. If you have shelter or a known safe terrain feature within easy reach (your tent, a vehicle, a hut, a low-angle ridge in trees), get to it…

**3.** `shelter` · *Choose a smart spot first* (hybrid=0.73, kw=0.72, vec=0.73) — `shelter.emergency-shelter#0`
...
**2.** `shelter` · *Practical use* (hybrid=0.68, kw=0.41, vec=0.79) — `shelter.bivy-sack#5`
...
**3.** `shelter` · *Condensation reality* (hybrid=0.65, kw=0.42, vec=0.75) — `shelter.bivy-sack#3`
...
**1.** `water` · *UV light* (hybrid=0.82, kw=0.94, vec=0.76) — `water.purification-methods#3`
...
**2.** `water` · *Overview* (hybrid=0.81, kw=1.00, vec=0.73) — `water.unsafe-water-sources#0`
...
**3.** `water` · *Sourcing into containers* (hybrid=0.57, kw=0.08, vec=0.79) — `water.water-storage#4`
...
- _top result hit keywords: ['filter', 'chemical']_
...
**1.** `water` · *Sources to avoid even after treatment* (hybrid=0.77, kw=0.82, vec=0.74) — `water.unsafe-water-sources#2`
...
**2.** `water` · *Practical priorities* (hybrid=0.70, kw=0.54, vec=0.77) — `water.unsafe-water-sources#4`
...
**3.** `food` · *Eat early, eat often* (hybrid=0.70, kw=0.97, vec=0.59) — `food.calorie-and-nutrition#3`
  > Breakfast and a steady stream of snacks fuel a day far better than a single big mid-day meal. Cold-weather camping especially benefits from a high-fat dinner consumed before getting in the sleeping bag — digestion produces internal heat.…

- _top result hit keywords: ['treat', 'filter', 'boil', 'contamination']_
...
**1.** `plants` · *Tradition vs safety* (hybrid=0.77, kw=1.00, vec=0.67) — `plants.pokeweed#6`
...
**2.** `water` · *Sources to avoid even after treatment* (hybrid=0.75, kw=0.73, vec=0.76) — `water.unsafe-water-sources#2`
  > Some contamination cannot be removed by filtration or boiling. Avoid these unless absolutely no alternative exists, and even then consider the trade-offs: - **Stagnant water with algae blooms** (green/red scum, foul smell). Cyanobacteria…

**3.** `water` · *Boiling* (hybrid=0.72, kw=0.49, vec=0.82) — `water.purification-methods#0`
...
**1.** `water` · *When to combine methods* (hybrid=0.80, kw=0.87, vec=0.77) — `water.purification-methods#4`
...
**3.** `gear-fixes` · *Backup treatment* (hybrid=0.78, kw=0.93, vec=0.72) — `gear-fixes.pad-and-filter-repair#5`
...
**2.** `water` · *Cryptosporidium ("Crypto")* (hybrid=0.83, kw=0.95, vec=0.77) — `water.water-borne-illness#1`
...
**3.** `water` · *Sources that look safer than they are* (hybrid=0.62, kw=0.44, vec=0.70) — `water.unsafe-water-sources#1`
...
**1.** `water` · *Rehydrating someone* (hybrid=0.86, kw=1.00, vec=0.80) — `water.dehydration#3`
...
**2.** `water` · *Recognizing it* (hybrid=0.77, kw=0.81, vec=0.76) — `water.dehydration#0`
...
**3.** `water` · *Electrolytes matter on long efforts* (hybrid=0.77, kw=0.79, vec=0.76) — `water.dehydration#2`
  > Sweating loses sodium along with water. Drinking only plain water during multi-hour efforts in heat can dilute blood sodium (hyponatremia) — symptoms mimic dehydration (headache, nausea, confusion) but adding more water makes it worse. E…
...
**2.** `food` · *Calories per day* (hybrid=0.79, kw=0.77, vec=0.81) — `food.calorie-and-nutrition#0`
...
**3.** `water` · *Daily intake guidelines* (hybrid=0.65, kw=0.40, vec=0.76) — `water.dehydration#1`
...
**1.** `water` · *Algae and cyanobacteria toxins* (hybrid=0.76, kw=1.00, vec=0.65) — `water.water-borne-illness#4`
...
**2.** `water` · *Sources to avoid even after treatment* (hybrid=0.73, kw=0.92, vec=0.64) — `water.unsafe-water-sources#2`
  > Some contamination cannot be removed by filtration or boiling. Avoid these unless absolutely no alternative exists, and even then consider the trade-offs: - **Stagnant water with algae blooms** (green/red scum, foul smell). Cyanobacteria…

**3.** `fire` · *Extinguishing* (hybrid=0.72, kw=1.00, vec=0.60) — `fire.fire-safety#2`
  > A fire is out only when the ashes are cold to the touch. Pour water on the fire, stir the ashes with a stick, pour again, and feel the entire area with your bare hand. If it's warm anywhere, repeat. Coals can stay alive under wet ash for…
...
**2.** `water` · *Melting technique* (hybrid=0.85, kw=1.00, vec=0.78) — `water.snowmelt-and-cold-water#1`
...
**3.** `water` · *Snow and ice* (hybrid=0.80, kw=0.90, vec=0.75) — `water.finding-water#3`
...
**2.** `water` · *Sources that look safer than they are* (hybrid=0.62, kw=0.55, vec=0.65) — `water.unsafe-water-sources#1`
  > - **High-elevation alpine streams**: usually clean of human pathogens but can carry giardia from upstream wildlife. Treat anyway. - **Snowmelt**: pure-looking but may carry whatever was on or in the snowpack. Treat. - **"Pristine"-lookin…

**3.** `water` · *Practical priorities* (hybrid=0.60, kw=0.36, vec=0.71) — `water.unsafe-water-sources#4`
  > 1. Best: moving water from springs or mid-stream, in remote drainages, away from grazing, mining, and downstream of campsites. 2. Acceptable: most alpine streams and lakes after proper treatment. 3. Last resort: cloudy or silty water — p…
...
**1.** `first-aid` · *Recognize life-threatening bleeding* (hybrid=0.78, kw=0.74, vec=0.79) — `first-aid.bleeding-control#0`
...
**2.** `first-aid` · *When to evacuate* (hybrid=0.72, kw=0.80, vec=0.68) — `first-aid.nosebleed#3`
...
**3.** `first-aid` · *What to avoid* (hybrid=0.70, kw=0.55, vec=0.77) — `first-aid.bleeding-control#3`
...
**2.** `fire` · *Stove types and trade-offs* (hybrid=0.76, kw=0.99, vec=0.66) — `fire.stove-cooking#0`
  > **Canister stoves** (Jetboil, MSR Pocket Rocket, etc.) burn pressurized propane/butane blend. Fast boil, simple to use, no priming. Performance drops sharply below ~25°F and at high altitude as pressure falls. Canisters can't be weighed …

**3.** `trip-basics` · *Minimize campfire impacts* (hybrid=0.72, kw=0.69, vec=0.73) — `trip-basics.leave-no-trace#3`
...
**1.** `first-aid` · *Recognize the signs* (hybrid=0.87, kw=0.98, vec=0.82) — `first-aid.hypothermia#0`
...
**2.** `first-aid` · *Rewarm gently* (hybrid=0.70, kw=0.60, vec=0.75) — `first-aid.hypothermia#2`
  > For mild hypothermia, sugary or warm (not hot) liquids and high-calorie food help — but only if the person can swallow safely. Skin-to-skin contact inside a sleeping bag is effective and safe. Chemical heat packs or warm water bottles wr…
...
**2.** `first-aid` · *Stroke — recognize with FAST* (hybrid=0.71, kw=0.68, vec=0.72) — `first-aid.heart-attack-stroke#3`
...
**3.** `first-aid` · *Treat heat exhaustion* (hybrid=0.70, kw=0.84, vec=0.64) — `first-aid.heat-illness#1`
...
**1.** `knots` · *How to tie it (twist method)* (hybrid=0.76, kw=1.00, vec=0.65) — `knots.alpine-butterfly#1`
...
**2.** `knots` · *Limitations* (hybrid=0.74, kw=0.95, vec=0.65) — `knots.munter-hitch#4`
  > - Twists the rope significantly during belay — old kinks and twists build up over a long pitch. - Less smooth than a dedicated belay device (BD ATC, Petzl GriGri). - Single-strand rappel only; not as smooth as devices for double-rope rap…

**3.** `first-aid` · *RICE for sprains* (hybrid=0.70, kw=0.61, vec=0.74) — `first-aid.sprains-fractures#1`
...
- _right domain but #3, not top_
- _top result hit keywords: ['ice']_
...
**1.** `wildlife` · *Behavior* (hybrid=0.70, kw=0.61, vec=0.74) — `wildlife.coral-snake#3`
  > Coral snakes are secretive and reluctant to bite — they're often found under leaf litter, logs, or in sandy soil. Most bites happen when people pick them up (sometimes mistaking them for harmless king snakes) or accidentally grab them wh…

**2.** `wildlife` · *Get away from the snake* (hybrid=0.69, kw=0.62, vec=0.73) — `wildlife.venomous-snake#1`
...
**3.** `wildlife` · *Avoiding snakes in the first place* (hybrid=0.69, kw=0.65, vec=0.70) — `wildlife.snakes#0`
...
- _top result hit keywords: ['bite']_
...
**1.** `first-aid` · *What NOT to do* (hybrid=0.84, kw=1.00, vec=0.77) — `first-aid.tick-removal#3`
...
**2.** `wildlife` · *Removing an attached tick* (hybrid=0.80, kw=0.94, vec=0.74) — `wildlife.ticks-disease-vectors#3`
...
**3.** `first-aid` · *Equipment* (hybrid=0.79, kw=0.88, vec=0.75) — `first-aid.tick-removal#1`
...
**1.** `first-aid` · *Prevent* (hybrid=0.83, kw=1.00, vec=0.75) — `first-aid.blisters#1`
...
**2.** `trip-basics` · *Common pre-trip mistakes* (hybrid=0.79, kw=0.99, vec=0.71) — `trip-basics.conditioning#2`
...
**3.** `gear-fixes` · *Hot spots and blisters mid-trip* (hybrid=0.79, kw=0.67, vec=0.84) — `gear-fixes.footwear-emergencies#3`
...
- _no expected topic keywords found in top result: ['tape', 'moleskin', 'drain', 'intact']_
...
**1.** `first-aid` · *Acute Mountain Sickness (AMS)* (hybrid=0.84, kw=0.97, vec=0.78) — `first-aid.altitude-illness#0`
...
**2.** `first-aid` · *Prevention and red flags* (hybrid=0.83, kw=0.97, vec=0.77) — `first-aid.altitude-illness#4`
...
**2.** `first-aid` · *Field treatment — deep frostbite* (hybrid=0.68, kw=0.37, vec=0.81) — `first-aid.frostbite#2`
...
**3.** `weather` · *Frostbite first aid (brief)* (hybrid=0.65, kw=0.37, vec=0.77) — `weather.wind-chill#4`
...
**1.** `wildlife` · *Anaphylaxis after a sting* (hybrid=0.78, kw=0.97, vec=0.70) — `wildlife.bees-wasps-hornets#5`
...
**2.** `first-aid` · *Anaphylaxis (severe allergic reaction)* (hybrid=0.72, kw=0.82, vec=0.68) — `first-aid.bites-stings#3`
...
**3.** `first-aid` · *Recognize it fast* (hybrid=0.71, kw=1.00, vec=0.59) — `first-aid.anaphylaxis#0`
...
- _right domain but #2, not top_
- _top result hit keywords: ['anaphylaxis', 'evacuate']_
...
**3.** `first-aid` · *Adult / older child: abdominal thrusts (Heimlich)* (hybrid=0.67, kw=0.37, vec=0.79) — `first-aid.choking#1`
...
**2.** `first-aid` · *Heart attack — what to do* (hybrid=0.77, kw=0.83, vec=0.74) — `first-aid.heart-attack-stroke#2`
  > 1. Have the person sit or lie in a position of comfort, often slightly upright. 2. Stop all exertion. Even walking is too much. 3. Activate emergency response: PLB, satellite SOS, call 911, send a runner. 4. If they have prescribed nitro…

**3.** `first-aid` · *This is not training* (hybrid=0.74, kw=0.60, vec=0.79) — `first-aid.cpr-basics#0`
...
**2.** `first-aid` · *Recognize life-threatening bleeding* (hybrid=0.68, kw=0.41, vec=0.80) — `first-aid.bleeding-control#0`
  > Life-threatening bleeding is bleeding that is spurting, pooling on the ground, soaking through clothing, or that won't stop with simple pressure. If the person becomes pale, cold, sweaty, confused, or weak, treat as life-threatening rega…

**3.** `first-aid` · *Stopping a typical nosebleed* (hybrid=0.65, kw=0.42, vec=0.75) — `first-aid.nosebleed#1`
...
**2.** `first-aid` · *Overview* (hybrid=0.60, kw=0.34, vec=0.71) — `first-aid.head-injury#0`
...
**3.** `navigation` · *Aim-off* (hybrid=0.59, kw=0.65, vec=0.56) — `navigation.handrails-and-aim-off#2`
...
**1.** `first-aid` · *Open fractures and dislocations* (hybrid=0.79, kw=1.00, vec=0.70) — `first-aid.sprains-fractures#3`
  > An open fracture (bone has broken through skin) is a high-infection-risk emergency. Cover the wound with sterile dressing, do not push bone back in, splint, and evacuate fast. For obvious dislocations of major joints (shoulder, hip, knee…

**2.** `wildlife` · *If gored* (hybrid=0.71, kw=0.81, vec=0.66) — `wildlife.bison#4`
...
**3.** `first-aid` · *Telling sprains from fractures in the field* (hybrid=0.66, kw=0.69, vec=0.65) — `first-aid.sprains-fractures#0`
  > You often cannot tell the difference without an X-ray. Treat any injury with significant pain, swelling, deformity, or inability to bear weight as a possible fracture. Signs that strongly suggest a fracture: visible deformity, bone protr…

- _top result hit keywords: ['splint']_
...
**2.** `water` · *E. coli and other bacteria* (hybrid=0.72, kw=0.71, vec=0.73) — `water.water-borne-illness#2`
...
**3.** `plants` · *What to do* (hybrid=0.72, kw=0.84, vec=0.67) — `plants.pokeweed#4`
  > Call Poison Control. Evacuate to medical care for any significant ingestion, especially in children. Treatment is supportive: IV fluids to replace what's lost in vomiting and diarrhea, antiemetics, electrolyte monitoring. There is no ant…
...
**1.** `first-aid` · *Position and protect* (hybrid=0.77, kw=1.00, vec=0.68) — `first-aid.anaphylaxis#2`
...
**2.** `first-aid` · *Once you have the person out* (hybrid=0.77, kw=0.85, vec=0.74) — `first-aid.drowning#2`
...
**3.** `first-aid` · *Cardiac arrest — different from heart attack* (hybrid=0.74, kw=0.84, vec=0.70) — `first-aid.heart-attack-stroke#5`
  > A heart attack is a circulation problem (blocked artery); the person is usually conscious. Cardiac arrest is an electrical problem (heart stops pumping effectively); the person collapses, unconscious, not breathing. Heart attacks can lea…

- _top result hit keywords: ['CPR']_
...
**2.** `wildlife` · *If you see a bear at a distance* (hybrid=0.86, kw=0.90, vec=0.84) — `wildlife.bear-encounter#0`
...
**3.** `wildlife` · *Bear spray* (hybrid=0.86, kw=1.00, vec=0.80) — `wildlife.bears#3`
...
**2.** `wildlife` · *Overview* (hybrid=0.84, kw=0.99, vec=0.77) — `wildlife.bear-spray#0`
...
**3.** `wildlife` · *If a moose charges* (hybrid=0.76, kw=0.80, vec=0.74) — `wildlife.moose-and-large-ungulates#2`
...
**1.** `wildlife` · *When to deploy* (hybrid=0.76, kw=0.67, vec=0.80) — `wildlife.bear-spray#3`
  > Deploy when a bear is charging, approaching aggressively, or has made contact. Do not spray a bear that is calmly grazing or moving away — back away yourself.

**2.** `wildlife` · *If you see a bear at a distance* (hybrid=0.76, kw=0.60, vec=0.83) — `wildlife.bears#2`
  > Stop and assess. If the bear hasn't seen you, back away quietly the way you came and detour widely. If the bear has seen you, identify yourself as human: speak calmly, wave your arms slowly. Do not run; running triggers chase instinct in…

**3.** `wildlife` · *If you see a bear at a distance* (hybrid=0.74, kw=0.52, vec=0.83) — `wildlife.bear-encounter#0`
  > Stop and stay calm. Most bears will avoid you if they hear you coming. Maintain distance — the National Park Service recommends staying at least 100 yards (about 91 m) from black and grizzly bears. Use binoculars or a telephoto lens to o…
...
**2.** `wildlife` · *Overview* (hybrid=0.84, kw=0.94, vec=0.80) — `wildlife.mountain-lion#0`
...
**3.** `wildlife` · *How rare attacks are — and why it matters* (hybrid=0.83, kw=0.95, vec=0.78) — `wildlife.mountain-lions#0`
...
**2.** `wildlife` · *If a moose charges* (hybrid=0.75, kw=0.57, vec=0.83) — `wildlife.moose-and-large-ungulates#2`
  > Run. This is the exception — unlike bears and mountain lions, you run from moose. Get behind a substantial tree (trees a moose can't easily knock down) or other solid obstacle. Moose charge to dislodge a threat, not to kill, and will usu…

**3.** `wildlife` · *Reducing the risk* (hybrid=0.75, kw=0.56, vec=0.83) — `wildlife.moose#5`
...
**1.** `wildlife` · *How rattlesnakes hunt and react* (hybrid=0.83, kw=0.90, vec=0.80) — `wildlife.rattlesnakes#1`
...
**2.** `wildlife` · *Range and species* (hybrid=0.83, kw=0.93, vec=0.78) — `wildlife.rattlesnakes#0`
...
**3.** `wildlife` · *Bites on dogs* (hybrid=0.80, kw=1.00, vec=0.72) — `wildlife.rattlesnakes#6`
  > Rattlesnake-bitten dogs typically present with rapid swelling, sometimes drooling, weakness, and shock. Get to a vet immediately — many vet clinics in rattlesnake country stock antivenom. Rattlesnake vaccines exist for dogs in high-expos…
...
**1.** `wildlife` · *Wolves* (hybrid=0.83, kw=1.00, vec=0.75) — `wildlife.coyotes-and-wolves#1`
...
**2.** `wildlife` · *Coyotes* (hybrid=0.78, kw=0.79, vec=0.78) — `wildlife.coyotes-and-wolves#0`
...
**3.** `wildlife` · *Foxes* (hybrid=0.75, kw=0.79, vec=0.73) — `wildlife.coyotes-and-wolves#2`
...
- _top result hit keywords: ['stand tall']_
...
**2.** `wildlife` · *Overview* (hybrid=0.80, kw=0.80, vec=0.79) — `wildlife.small-raiders#0`
...
**3.** `wildlife` · *Threats from small raiders* (hybrid=0.71, kw=0.64, vec=0.74) — `wildlife.small-raiders#1`
...
**2.** `wildlife` · *In the water* (hybrid=0.65, kw=0.43, vec=0.74) — `wildlife.alligators-and-crocodiles#4`
...
**3.** `wildlife` · *If attacked* (hybrid=0.64, kw=0.34, vec=0.78) — `wildlife.alligators-and-crocodiles#5`
  > Fight aggressively. Punch, gouge eyes, hit the snout, push fingers into the nostrils. Alligators rely on their bite — if you make the attack costly enough, many will release. Don't play dead. Once free, get away from the water as fast as…
...
**2.** `wildlife` · *Removing a stinger* (hybrid=0.67, kw=0.37, vec=0.80) — `wildlife.bees-wasps-hornets#3`
...
**3.** `wildlife` · *The only medically dangerous US scorpion* (hybrid=0.67, kw=0.44, vec=0.77) — `wildlife.bark-scorpion#0`
...
**2.** `wildlife` · *Removing an attached tick* (hybrid=0.78, kw=0.94, vec=0.71) — `wildlife.ticks-disease-vectors#3`
  > Use fine-tipped tweezers, grasp the tick as close to the skin as possible, pull straight up with steady firm pressure. Don't twist or jerk — mouth parts may break off and stay in the skin (these don't transmit disease but can cause local…

**3.** `first-aid` · *Overview* (hybrid=0.77, kw=0.91, vec=0.71) — `first-aid.tick-removal#0`
  > Ticks transmit Lyme disease, anaplasmosis, babesiosis, Rocky Mountain spotted fever, and several other illnesses. The risk of disease transmission rises sharply after a tick has been attached for 24–36 hours. Prompt, correct removal dram…
...
**2.** `wildlife` · *Prevention* (hybrid=0.81, kw=0.91, vec=0.76) — `wildlife.bats-and-rabies#5`
...
**3.** `wildlife` · *When to start PEP without bat available* (hybrid=0.77, kw=0.91, vec=0.71) — `wildlife.bats-and-rabies#4`
...
**1.** `plants` · *Identification* (hybrid=0.89, kw=0.99, vec=0.84) — `plants.poison-ivy-oak-sumac#0`
...
**2.** `plants` · *If you contact it* (hybrid=0.84, kw=0.97, vec=0.78) — `plants.poison-ivy-oak-sumac#2`
...
**3.** `plants` · *Burning is dangerous* (hybrid=0.83, kw=1.00, vec=0.76) — `plants.poison-ivy-oak-sumac#3`
...
**1.** `plants` · *Identification* (hybrid=0.82, kw=1.00, vec=0.75) — `plants.foxglove#1`
  > A tall plant, 2–6 feet, with a single unbranched stem rising from a basal rosette of large fuzzy gray-green leaves. Flowers form a dense one-sided spike of bell-shaped blossoms in shades of purple, pink, white, or yellow, each with darke…

**2.** `plants` · *Poison hemlock identification* (hybrid=0.81, kw=0.99, vec=0.73) — `plants.poison-hemlock#1`
...
**3.** `plants` · *Poison hemlock (Conium maculatum) — historic poison of Socrates* (hybrid=0.74, kw=0.72, vec=0.74) — `plants.dangerous-plants#3`
...
- _no expected topic keywords found in top result: ['poison hemlock', 'deadly', 'carrot family']_
...
**1.** `plants` · *Burning is dangerous* (hybrid=0.81, kw=0.85, vec=0.80) — `plants.poison-ivy-oak-sumac#3`
  > Never burn poison ivy, oak, or sumac. The urushiol vaporizes into smoke and can be inhaled, causing reactions in the lungs and airways that have killed people. Pull plants by hand only in heavy gloves you can dispose of, and bag the plan…

**2.** `plants` · *Overview* (hybrid=0.80, kw=0.88, vec=0.77) — `plants.dangerous-plants#0`
...
**3.** `plants` · *Why it matters* (hybrid=0.80, kw=1.00, vec=0.71) — `plants.wild-parsnip#0`
...
**1.** `plants` · *Identification* (hybrid=0.80, kw=0.94, vec=0.73) — `plants.manchineel#1`
...
**2.** `plants` · *Where it lives* (hybrid=0.79, kw=1.00, vec=0.70) — `plants.manchineel#0`
...
**3.** `plants` · *Manchineel (Hippomane mancinella) — only in southern Florida and the Caribbean* (hybrid=0.66, kw=0.42, vec=0.76) — `plants.dangerous-plants#4`
...
- _top result hit keywords: ['manchineel', 'toxic', 'do not touch']_
...
**2.** `weather` · *When you're at risk* (hybrid=0.80, kw=0.98, vec=0.73) — `weather.lightning-safety#0`
...
**3.** `first-aid` · *Prevention is the real answer* (hybrid=0.72, kw=0.70, vec=0.72) — `first-aid.lightning-strike-injury#9`
  > See weather.lightning-safety for the prevention strategy. If you can hear thunder, lightning is in strike range. Get out of exposed terrain. Assume lightning position if caught in open. Spread out from companions so one strike doesn't in…
...
**2.** `weather` · *Warning signs* (hybrid=0.68, kw=0.37, vec=0.81) — `weather.flash-floods#2`
...
**3.** `weather` · *The best shelter* (hybrid=0.64, kw=0.39, vec=0.75) — `weather.lightning-safety#1`
  > A fully enclosed building or a hard-topped vehicle (windows up). The metal frame of a car conducts current around the occupants — pickup trucks with cabs work; open Jeeps and convertibles do not. In the backcountry where no shelter exist…
...
**2.** `weather` · *Get the warnings before they arrive* (hybrid=0.58, kw=0.34, vec=0.68) — `weather.tornado-and-severe#4`
...
**3.** `first-aid` · *If you must move them* (hybrid=0.57, kw=0.46, vec=0.62) — `first-aid.spinal-injury#2`
...
**1.** `weather` · *Why they kill so many people* (hybrid=0.80, kw=0.77, vec=0.82) — `weather.flash-floods#0`
...
**2.** `weather` · *Action if caught* (hybrid=0.76, kw=0.71, vec=0.77) — `weather.flash-floods#3`
...
**1.** `weather` · *Recognize the threat* (hybrid=0.84, kw=0.95, vec=0.80) — `weather.tornado-and-severe#0`
...
**2.** `weather` · *Best shelter outdoors* (hybrid=0.81, kw=0.87, vec=0.78) — `weather.tornado-and-severe#1`
...
**2.** `weather` · *Prevention* (hybrid=0.79, kw=0.78, vec=0.80) — `weather.whiteout-conditions#5`
...
**3.** `weather` · *Don't navigate to "where I think I am"* (hybrid=0.72, kw=0.74, vec=0.71) — `weather.whiteout-conditions#4`
...
**1.** `weather` · *Overview* (hybrid=0.85, kw=0.87, vec=0.84) — `weather.wind-chill#0`
...
**1.** `plants` · *Treating burns* (hybrid=0.86, kw=1.00, vec=0.81) — `plants.giant-hogweed#3`
...
**2.** `weather` · *Clothing as the primary defense* (hybrid=0.86, kw=1.00, vec=0.80) — `weather.sun-and-uv#2`
...
**3.** `weather` · *Eyes* (hybrid=0.78, kw=0.93, vec=0.71) — `weather.sun-and-uv#3`
  > UV damage to the eyes is cumulative and can cause "snow blindness" (photokeratitis) — painful, temporary, but disabling. Wear sunglasses with full UV protection (UV 400 or "100% UVA/UVB" labeled). At high altitude or on snow, use wraparo…

- _right domain but #2, not top_
- _no expected topic keywords found in top result: ['SPF', 'hat', 'lip', 'reapply']_
...
**2.** `weather` · *Terrain that slides* (hybrid=0.81, kw=0.97, vec=0.74) — `weather.avalanche-awareness#1`
...
**3.** `weather` · *This is an awareness primer, not training* (hybrid=0.72, kw=0.72, vec=0.73) — `weather.avalanche-awareness#0`
...
**1.** `navigation` · *STOP — Stop, Think, Observe, Plan* (hybrid=0.68, kw=0.81, vec=0.63) — `navigation.lost-and-signaling#0`
  > The first response to "I might be lost" is STOP. Stop moving. Sit down. Drink water. Think — when did you last know exactly where you were, and what direction have you been walking since? Observe — what landmarks can you see? Plan — do n…

**2.** `trip-basics` · *S — Stop* (hybrid=0.66, kw=0.74, vec=0.63) — `trip-basics.if-you-get-lost#1`
...
**3.** `first-aid` · *Acute Mountain Sickness (AMS)* (hybrid=0.65, kw=1.00, vec=0.50) — `first-aid.altitude-illness#0`
  > AMS typically appears 6–12 hours after arriving above ~8,000 ft (2,400 m). Symptoms: headache, nausea or loss of appetite, fatigue beyond what the day's effort explains, dizziness, and disturbed sleep. AMS feels like a hangover at altitu…

- _top result hit keywords: ['STOP']_
...
**1.** `navigation` · *Taking a bearing from the map* (hybrid=0.91, kw=0.95, vec=0.89) — `navigation.map-and-compass#1`
...
**3.** `navigation` · *Declination* (hybrid=0.85, kw=0.89, vec=0.84) — `navigation.map-and-compass#0`
...
**2.** `navigation` · *Taking a bearing from the map* (hybrid=0.57, kw=0.48, vec=0.61) — `navigation.map-and-compass#1`
  > 1. Lay the long edge of the compass baseplate along the line from where you are to where you want to go on the map. 2. Rotate the compass housing until the orienting lines inside it line up with the map's north–south grid lines (north ar…
...
**2.** `navigation` · *Taking a bearing from the map* (hybrid=0.84, kw=0.84, vec=0.84) — `navigation.map-and-compass#1`
  > 1. Lay the long edge of the compass baseplate along the line from where you are to where you want to go on the map. 2. Rotate the compass housing until the orienting lines inside it line up with the map's north–south grid lines (north ar…

**3.** `navigation` · *Declination* (hybrid=0.82, kw=0.82, vec=0.82) — `navigation.map-and-compass#0`
  > A compass needle points to magnetic north; maps are drawn to true north (or grid north, in some military maps). The angle between them is "declination," varies by location, and changes slowly over years. Your topo map's margin states the…
...
**2.** `navigation` · *Declination* (hybrid=0.74, kw=0.73, vec=0.74) — `navigation.map-and-compass#0`
  > A compass needle points to magnetic north; maps are drawn to true north (or grid north, in some military maps). The angle between them is "declination," varies by location, and changes slowly over years. Your topo map's margin states the…

**3.** `navigation` · *Taking a bearing from the map* (hybrid=0.63, kw=0.53, vec=0.67) — `navigation.map-and-compass#1`
  > 1. Lay the long edge of the compass baseplate along the line from where you are to where you want to go on the map. 2. Rotate the compass housing until the orienting lines inside it line up with the map's north–south grid lines (north ar…
...
**1.** `trip-basics` · *O — Observe* (hybrid=0.85, kw=1.00, vec=0.78) — `trip-basics.if-you-get-lost#3`
...
**2.** `navigation` · *What GPS does and doesn't do* (hybrid=0.84, kw=0.87, vec=0.82) — `navigation.gps-basics#0`
...
**3.** `navigation` · *Phone-as-GPS realities* (hybrid=0.78, kw=0.76, vec=0.79) — `navigation.gps-basics#4`
...
- _right domain but #2, not top_
- _top result hit keywords: ['satellite']_
...
**1.** `navigation` · *Pace counting* (hybrid=0.80, kw=0.94, vec=0.74) — `navigation.pace-and-time#1`
  > Find your personal pace count: walk a measured 100 meters on flat ground at a normal carrying-pack speed and count steps on one foot (every time the left foot lands). Most adults are between 60–70 paces per 100 meters. Repeat several tim…

**2.** `navigation` · *Using pace count* (hybrid=0.77, kw=1.00, vec=0.67) — `navigation.pace-and-time#2`
  > On a bearing through poor visibility (whiteout, fog, dense forest), pace-count to measure distance covered. Combined with a compass bearing, this gives you a position estimate. After every 100 meters of pace count, slide a bean from one …

**3.** `navigation` · *Naismith's Rule (and improvements)* (hybrid=0.71, kw=0.81, vec=0.67) — `navigation.pace-and-time#0`
...
- _top result hit keywords: ['pace', 'time']_
...
**1.** `navigation` · *Using pace count* (hybrid=0.74, kw=1.00, vec=0.63) — `navigation.pace-and-time#2`
  > On a bearing through poor visibility (whiteout, fog, dense forest), pace-count to measure distance covered. Combined with a compass bearing, this gives you a position estimate. After every 100 meters of pace count, slide a bean from one …

**2.** `navigation` · *Boxing obstacles* (hybrid=0.66, kw=0.55, vec=0.71) — `navigation.handrails-and-aim-off#3`
...
**3.** `weather` · *If you must travel* (hybrid=0.66, kw=0.67, vec=0.66) — `weather.whiteout-conditions#3`
...
- _top result hit keywords: ['bearing', 'pace']_
...
**1.** `navigation` · *Active signaling* (hybrid=0.81, kw=1.00, vec=0.73) — `navigation.lost-and-signaling#2`
  > Three of anything is the international distress signal: three whistle blasts, three shots, three smoky signal fires arranged in a triangle, three blinks of a mirror. Carry a real whistle — it works far longer than your voice. A signal mi…

**2.** `fire` · *Combine with other signals* (hybrid=0.81, kw=0.97, vec=0.73) — `fire.signal-fire#5`
  > Signal fires work alongside other signals, not in isolation. Bright-colored fabric (rain jacket, sleeping bag) spread on open ground or in trees doubles your visibility. Signal mirror works against any clear-sky sun. Whistle blasts repea…

**3.** `trip-basics` · *Signaling* (hybrid=0.76, kw=0.82, vec=0.73) — `trip-basics.if-you-get-lost#5`
  > - Three of anything is the universal distress signal: three whistle blasts, three fires in a triangle, three flashes. - A whistle carries much further than a voice and uses much less energy. Carry one on a lanyard. - A signal mirror flas…

- _top result hit keywords: ['whistle', 'mirror', 'three']_
...
**1.** `navigation` · *How altimeters work* (hybrid=0.86, kw=0.92, vec=0.84) — `navigation.altimeter-use#0`
...
**2.** `navigation` · *When altimeter matters most* (hybrid=0.82, kw=1.00, vec=0.74) — `navigation.altimeter-use#5`
...
**3.** `navigation` · *Weather predictor* (hybrid=0.81, kw=0.90, vec=0.77) — `navigation.altimeter-use#3`
...
**1.** `food` · *Bear hangs* (hybrid=0.91, kw=1.00, vec=0.88) — `food.bear-aware-food-storage#2`
...
**2.** `wildlife` · *Bear hang (PCT method)* (hybrid=0.85, kw=0.96, vec=0.80) — `wildlife.food-storage#3`
...
**3.** `food` · *Ursack and soft-sided alternatives* (hybrid=0.78, kw=0.77, vec=0.78) — `food.bear-aware-food-storage#3`
...
**1.** `wildlife` · *Avoiding an encounter* (hybrid=0.88, kw=1.00, vec=0.83) — `wildlife.bears#1`
...
**3.** `wildlife` · *Overview* (hybrid=0.78, kw=0.72, vec=0.81) — `wildlife.food-storage#0`
...
**2.** `food` · *Calorie targets by trip type* (hybrid=0.78, kw=0.89, vec=0.74) — `food.meal-planning#0`
...
**3.** `food` · *Weight per day targets* (hybrid=0.58, kw=0.32, vec=0.69) — `food.meal-planning#1`
...
**2.** `food` · *Meal structure* (hybrid=0.73, kw=0.73, vec=0.73) — `food.meal-planning#2`
...
**3.** `food` · *Cooking temperatures* (hybrid=0.72, kw=0.76, vec=0.71) — `food.food-safety-hygiene#2`
...
**2.** `first-aid` · *Stopping the spread* (hybrid=0.75, kw=0.73, vec=0.76) — `first-aid.gi-illness#3`
...
**3.** `food` · *Communal vs personal food* (hybrid=0.72, kw=0.75, vec=0.70) — `food.food-safety-hygiene#1`
...
**1.** `first-aid` · *Why this matters* (hybrid=0.81, kw=1.00, vec=0.73) — `first-aid.gi-illness#0`
  > A camper with bad diarrhea and vomiting can be dangerously dehydrated within 12 hours, especially in heat or at altitude. GI illness also spreads fast through a group sharing a kitchen — one infected hand on a shared spoon takes out a wh…

**2.** `plants` · *Amatoxin poisoning timeline* (hybrid=0.74, kw=0.83, vec=0.70) — `plants.wild-mushrooms#3`
...
**3.** `plants` · *What to do* (hybrid=0.74, kw=0.74, vec=0.73) — `plants.pokeweed#4`
  > Call Poison Control. Evacuate to medical care for any significant ingestion, especially in children. Treatment is supportive: IV fluids to replace what's lost in vomiting and diarrhea, antiemetics, electrolyte monitoring. There is no ant…
...
**1.** `knots` · *When NOT to use* (hybrid=0.89, kw=1.00, vec=0.84) — `knots.bowline-on-a-bight#7`
  > Don't use any bowline variant for life-safety climbing tie-in — use a figure-eight follow-through. The figure-eight is harder to tie wrong and stays tied under repeated loading. Bowlines have specific advantages (easier to untie after lo…

**2.** `knots` · *What it does* (hybrid=0.87, kw=0.87, vec=0.88) — `knots.bowline#0`
...
**3.** `knots` · *When not to use it* (hybrid=0.87, kw=0.92, vec=0.85) — `knots.bowline#3`
...
**2.** `knots` · *What it does* (hybrid=0.82, kw=0.89, vec=0.78) — `knots.bowline#0`
  > The bowline makes a fixed loop at the end of a rope that does not slip or tighten under load — useful for tying around a tree, around your waist in a self-rescue, or to a fixed point. It can be untied easily even after heavy loading, whi…

**3.** `knots` · *Best uses* (hybrid=0.80, kw=0.79, vec=0.80) — `knots.square-knot#3`
...
**2.** `knots` · *When to use it* (hybrid=0.82, kw=0.87, vec=0.79) — `knots.taut-line-hitch#2`
...
**3.** `knots` · *What it does* (hybrid=0.76, kw=0.79, vec=0.75) — `knots.trucker-hitch#0`
...
**2.** `knots` · *When to use what* (hybrid=0.79, kw=0.68, vec=0.84) — `knots.two-half-hitches#3`
  > - Tying a rope to a tree or post: two half hitches, or a bowline if you want a fixed loop. - Joining two ropes for tension (clothesline, ridgeline extension): sheet bend. - Joining two ropes for life-safety: not in this pack — take a cla…

**3.** `knots` · *How to tie it* (hybrid=0.78, kw=0.70, vec=0.82) — `knots.double-fisherman#1`
...
**2.** `knots` · *When to use it* (hybrid=0.83, kw=0.93, vec=0.79) — `knots.taut-line-hitch#2`
  > Use the taut-line hitch any time you might need to tighten a line — tent guyouts, ridgelines, tarp corners in changing weather. It holds under tension but releases when you grip the knot and slide it. On slick modern cord (line-lock plas…

**3.** `shelter` · *A-frame (symmetric ridgeline)* (hybrid=0.77, kw=0.80, vec=0.76) — `shelter.tarp-configurations#0`
  > The A-frame is the most weather-resistant general-purpose tarp pitch. Run a ridgeline rope between two trees about a body length apart, drape the tarp over it, and stake out the four corners at roughly 45° angles. Tighten guylines so the…
...
**2.** `knots` · *Shear lashing — for joining parallel poles* (hybrid=0.75, kw=0.78, vec=0.74) — `knots.lashings#3`
...
**3.** `gear-fixes` · *Frame damage* (hybrid=0.62, kw=0.64, vec=0.62) — `gear-fixes.pack-strap-repair#4`
...
**1.** `knots` · *Recognize a correct square knot* (hybrid=0.86, kw=0.92, vec=0.83) — `knots.square-knot#2`
...
**2.** `knots` · *Sizing the tails* (hybrid=0.84, kw=1.00, vec=0.77) — `knots.double-fisherman#3`
...
**3.** `knots` · *The slipping problem* (hybrid=0.82, kw=0.95, vec=0.76) — `knots.water-knot#3`
  > Water knots have a documented failure mode: they slip slowly under cyclic loading, working themselves loose over time. Always leave 3"+ tails. Inspect slings before each climb — water knots with tails shorter than 2" should be retied or …
...
**1.** `gear-fixes` · *Boot blowout* (hybrid=0.84, kw=0.98, vec=0.78) — `gear-fixes.field-repairs#2`
...
**2.** `gear-fixes` · *Replace before you have to* (hybrid=0.82, kw=1.00, vec=0.75) — `gear-fixes.footwear-emergencies#5`
...
**3.** `gear-fixes` · *Sole separation* (hybrid=0.79, kw=0.75, vec=0.81) — `gear-fixes.footwear-emergencies#0`
  > The most common boot failure on long trips: the sole detaches from the upper, usually starting at the toe. Field fix: clean both surfaces with a rag, dry as completely as possible, apply cyanoacrylate (super glue) or polyurethane glue al…
...
**2.** `gear-fixes` · *Torn pack body* (hybrid=0.76, kw=0.69, vec=0.79) — `gear-fixes.pack-strap-repair#3`
...
**3.** `gear-fixes` · *Sewing* (hybrid=0.74, kw=0.93, vec=0.66) — `gear-fixes.repair-kit#2`
...
**1.** `gear-fixes` · *Specific spare parts* (hybrid=0.88, kw=1.00, vec=0.83) — `gear-fixes.repair-kit#5`
...
**2.** `gear-fixes` · *Broken tent pole* (hybrid=0.87, kw=0.84, vec=0.88) — `gear-fixes.field-repairs#1`
...
**3.** `shelter` · *How to pitch a tent step by step* (hybrid=0.70, kw=0.54, vec=0.76) — `shelter.tent-setup#0`
  > The standard order for setting up most freestanding backpacking tents:  1. **Pick a flat spot** clear of sharp objects, away from widow-makers (dead branches and trees), and not in a low spot where water would pool. 2. **Lay down the foo…

- _top result hit keywords: ['splint', 'sleeve']_
...
**2.** `gear-fixes` · *Patch the leak* (hybrid=0.79, kw=0.80, vec=0.78) — `gear-fixes.pad-and-filter-repair#1`
...
**2.** `gear-fixes` · *Frame damage* (hybrid=0.83, kw=0.97, vec=0.77) — `gear-fixes.pack-strap-repair#4`
  > Internal-frame packs sometimes break their aluminum stays. A bent stay can sometimes be straightened by hand. A broken stay needs a splint — a stick of similar length lashed to the broken stay with cord, with the broken pieces aligned. T…
...
**2.** `trip-basics` · *Bring them every time* (hybrid=0.75, kw=0.62, vec=0.80) — `trip-basics.ten-essentials#3`
...
**3.** `trip-basics` · *Core categories* (hybrid=0.75, kw=0.82, vec=0.72) — `trip-basics.packing-list#1`
...
**2.** `trip-basics` · *Plan ahead and prepare* (hybrid=0.68, kw=0.90, vec=0.59) — `trip-basics.leave-no-trace#0`
...
**3.** `fire` · *Leave no trace* (hybrid=0.66, kw=0.69, vec=0.65) — `fire.fire-safety#3`
...
**2.** `trip-basics` · *Decide whether to cross* (hybrid=0.77, kw=0.75, vec=0.78) — `trip-basics.river-crossings#2`
...
**3.** `trip-basics` · *Scout the crossing* (hybrid=0.76, kw=0.72, vec=0.77) — `trip-basics.river-crossings#1`
...
**2.** `navigation` · *Stay put or self-rescue* (hybrid=0.59, kw=0.27, vec=0.73) — `navigation.lost-and-signaling#1`
  > If you've told someone your trip plan and expected return, staying put is usually safer than moving. Searchers start at the last known point and work outward; if you keep moving, they may have to re-search areas already cleared. Stay whe…

**3.** `navigation` · *Using altitude to navigate* (hybrid=0.58, kw=0.56, vec=0.60) — `navigation.altimeter-use#2`
...
- _domain: top3 = ['first-aid', 'navigation', 'navigation'], expected 'trip-basics'_
...
**2.** `trip-basics` · *Simple training plan* (hybrid=0.80, kw=0.66, vec=0.86) — `trip-basics.conditioning#1`
...
**3.** `trip-basics` · *Common pre-trip mistakes* (hybrid=0.79, kw=0.79, vec=0.80) — `trip-basics.conditioning#2`
  > "I'll get into shape on the trail" — no, the trail will get you into shape if you survive the first 3 days. Old injuries flare up under loaded hiking; address them before the trip. New boots not broken in cause blister disasters; walk in…
...
**2.** `trip-basics` · *Use a written checklist* (hybrid=0.83, kw=0.88, vec=0.80) — `trip-basics.packing-list#0`
...
**3.** `gear-fixes` · *Frame damage* (hybrid=0.77, kw=0.94, vec=0.70) — `gear-fixes.pack-strap-repair#4`
  > Internal-frame packs sometimes break their aluminum stays. A bent stay can sometimes be straightened by hand. A broken stay needs a splint — a stick of similar length lashed to the broken stay with cord, with the broken pieces aligned. T…
...
**2.** `trip-basics` · *Set a turnaround time before you leave* (hybrid=0.77, kw=0.83, vec=0.74) — `trip-basics.when-to-turn-back#1`
...
**3.** `navigation` · *Boxing obstacles* (hybrid=0.75, kw=0.99, vec=0.65) — `navigation.handrails-and-aim-off#3`
  > To navigate around a lake, cliff, or impassable bog while keeping your bearing: turn 90° at the obstacle, count paces along the new direction until past the obstacle, turn 90° to resume original direction, walk past the obstacle, turn 90…
...
**1.** `fire` · *Char cloth and tinderboxes* (hybrid=0.82, kw=1.00, vec=0.74) — `fire.primitive-fire-starting#6`
  > A char cloth is partly-burned cotton cloth. Pack a small cotton scrap in a tin (an Altoid tin works), make a small hole in the lid, throw the tin in a hot fire until the smoke from the hole stops, let it cool. The result inside is char c…

**2.** `fire` · *Ferro rods (fire steels)* (hybrid=0.64, kw=0.64, vec=0.65) — `fire.ignition-sources#3`
  > A ferrocerium rod scraped with a steel striker produces 3,000°F+ sparks that ignite tinder. Advantages: works wet, works cold, lasts thousands of strikes per rod, no fuel to run out. Disadvantage: needs good tinder and practice. The skil…

**3.** `fire` · *The three sizes* (hybrid=0.61, kw=0.48, vec=0.67) — `fire.tinder-and-fire-starting#1`
...
- _top result hit keywords: ['tinder', 'char cloth']_
...
**2.** `first-aid` · *Blunt impact / penetrating injury* (hybrid=0.57, kw=0.41, vec=0.64) — `first-aid.eye-injuries#3`
...
**3.** `gear-fixes` · *Multitool or knife* (hybrid=0.52, kw=0.19, vec=0.66) — `gear-fixes.repair-kit#6`
...
**1.** `trip-basics` · *Why crossings are dangerous* (hybrid=0.82, kw=1.00, vec=0.74) — `trip-basics.river-crossings#0`
  > Moving water is far stronger than its surface suggests. Water knee-deep at 5 mph exerts about 65 pounds of force on your shins; at thigh-deep it's enough to sweep most adults off their feet. Cold water (most mountain streams) shuts down …

**2.** `trip-basics` · *Scout the crossing* (hybrid=0.79, kw=0.83, vec=0.77) — `trip-basics.river-crossings#1`
  > Walk 50–100 meters up and downstream looking for the best line. Best is usually: wide and shallow (flow spreads out and slows), gravel-bed (firmer footing than mud or moss-covered rocks), no log jams or strainers downstream (those drown …

**3.** `trip-basics` · *Decide whether to cross* (hybrid=0.70, kw=0.60, vec=0.74) — `trip-basics.river-crossings#2`
  > Don't cross water above your thighs in significant current. Don't cross moving water that's brown, churning, or carrying debris — those signal a flood pulse, often from upstream rain. Don't cross if there's no clear exit on the far side,…
...
**1.** `first-aid` · *Foot care discipline* (hybrid=0.85, kw=0.97, vec=0.80) — `first-aid.trench-foot#5`
...
**2.** `first-aid` · *What it is* (hybrid=0.84, kw=1.00, vec=0.76) — `first-aid.trench-foot#0`
...
**3.** `first-aid` · *In rain pants and gaiters* (hybrid=0.83, kw=0.91, vec=0.80) — `first-aid.trench-foot#6`
...
**1.** `wildlife` · *Prevention — what actually works* (hybrid=0.77, kw=1.00, vec=0.67) — `wildlife.mosquito-disease#2`
  > **Repellents on skin**: EPA-registered DEET (20–30% concentration), picaridin (20%), oil of lemon eucalyptus (10–30%), IR3535. DEET is the gold standard; picaridin works almost as well and doesn't damage plastics. Apply every 4 hours whe…

**2.** `wildlife` · *Prevention: clothing and chemistry* (hybrid=0.72, kw=0.86, vec=0.66) — `wildlife.ticks-disease-vectors#1`
  > The most effective combination: long pants tucked into socks, long-sleeve shirts, light colors so ticks are visible, plus permethrin-treated clothing (treats fabric, lasts through about 6 washes, kills ticks on contact). On exposed skin,…

**3.** `wildlife` · *Why they matter more than the bite* (hybrid=0.66, kw=0.52, vec=0.72) — `wildlife.ticks-disease-vectors#0`
...
- _top result hit keywords: ['mosquito', 'permethrin', 'DEET']_
...
**1.** `first-aid` · *Recognize the signs* (hybrid=0.68, kw=1.00, vec=0.54) — `first-aid.hypothermia#0`
  > Mild hypothermia (~95–93°F core): shivering, slurred speech, clumsy hands, the "umbles" (mumbles, fumbles, stumbles). The person can usually still respond and follow simple instructions. Moderate hypothermia (~92–86°F): shivering may sto…

**2.** `water` · *Rehydrating someone* (hybrid=0.65, kw=0.91, vec=0.54) — `water.dehydration#3`
  > For mild to moderate dehydration in a conscious person who can swallow, give small sips of cool electrolyte solution or water frequently — gulping triggers nausea. Add a pinch of salt and a small spoon of sugar to a liter of water if no …

**3.** `wildlife` · *Children and severe cases* (hybrid=0.64, kw=0.56, vec=0.68) — `wildlife.bark-scorpion#4`
...
- _top result hit keywords: ['hypothermia']_
