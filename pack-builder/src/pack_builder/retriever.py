...
class TrailpackReader:
    ...
    def _keyword_scores(self, query: str, limit: int = 50) -> dict[str, float]:
        # Sanitize for FTS5: strip control chars and quotes, filter stopwords,
        # require terms to be 2+ chars. Stopwords like "I", "a", "the" otherwise
        # inflate BM25 scores for unrelated documents.
        raw_terms = query.replace('"', "").lower().split()
        terms = [t for t in raw_terms if len(t) > 1 and t not in _STOPWORDS]
        if not terms:
            # Fall back to original tokens if everything was filtered (rare)
